...
const failures = [];
const requiredRoot = ["README.md", "README.zh-CN.md", "LICENSE", "NOTICE", "THIRD_PARTY_NOTICES.txt", "CONTRIBUTING.md", "SECURITY.md", "CODE_OF_CONDUCT.md", "SUPPORT.md", "GOVERNANCE.md", "TRADEMARKS.md", "CHANGELOG.md", "RELEASE_CHECKLIST.md"];
...
if (!(await exists(siteRoot))) failures.push("site artifact is missing; run npm run site:build");
  ...
  for (const file of htmlFiles) {
    ...
    for (const match of html.matchAll(/\s(?:href|src)="([^"]+)"/g)) {
      ...
      const configuredBase = `${process.env.ANYTTY_SITE_BASE_PATH ?? "/anytty"}/`.replace("//", "/");

...
> **Project status:** pre-release and under active development. There are no stable releases, compatibility guarantees, published packages, or production-readiness claims yet. Build from source for development and evaluation, and expect protocols and configuration to change before the first release.
...
Android release validation additionally needs Java 21 and the Android SDK. iOS builds require macOS, Xcode, and the prerequisites in [clients/mobile/ios/README.md](clients/mobile/ios/README.md). The repository does not currently publish prebuilt packages.
...
Use `./.artifacts/bin/anytty --help` and the [quick-start guide](https://lozzo.github.io/anytty/docs/quick-start.html) for the commands implemented by the current checkout. Mobile clients add a daemon by scanning the one-time service QR code; they do not sign in or discover account devices automatically.
...
- [Documentation site](https://lozzo.github.io/anytty/) and [documentation source](site/README.md)
- [Contributing](CONTRIBUTING.md), [governance](GOVERNANCE.md), and [code of conduct](CODE_OF_CONDUCT.md)
- [Security policy](SECURITY.md) and [private vulnerability reporting](https://github.com/lozzo/anytty/security/advisories/new)

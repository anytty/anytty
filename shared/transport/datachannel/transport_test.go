...
func TestTransportSendFailureClosesTransport(t *testing.T) {
	channel := &failingSendChannel{done: make(chan struct{})}
	transport := New(channel)
	if err := transport.Send([]byte("frame")); !errors.Is(err, io.ErrUnexpectedEOF) {
		t.Fatalf("send error = %v", err)
	}
	select {
	case <-transport.Done():
	case <-time.After(time.Second):
		t.Fatal("send failure left transport alive")
	}
	if channel.closeCalls != 1 {
		t.Fatalf("channel close calls = %d, want 1", channel.closeCalls)
	}
}

func TestTransportSendDrainTimeoutClosesTransport(t *testing.T) {
	clientChannel, _ := newFakeChannelPair()
	clientChannel.setBufferedAmount(defaultSendBufferHigh + 1)
	transport := New(clientChannel)
	transport.drainTimeout = 10 * time.Millisecond

	if err := transport.Send([]byte("frame")); !errors.Is(err, context.DeadlineExceeded) {
...
func (*failingSendChannel) SetMessageHandler(func([]byte))       {}
func (*failingSendChannel) SetCloseHandler(func())               {}
func (*failingSendChannel) BufferedAmount() uint64               { return 0 }
func (*failingSendChannel) SetBufferedAmountLowThreshold(uint64) {}
func (*failingSendChannel) SetBufferedAmountLowHandler(func())   {}
func (*failingSendChannel) Send([]byte) error                    { return io.ErrUnexpectedEOF }
func (channel *failingSendChannel) Close() error {
	channel.closeCalls++
	channel.closeOnce.Do(func() { close(channel.done) })
	return nil
}

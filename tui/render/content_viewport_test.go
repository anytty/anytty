...
func TestTerminalLiveIgnoresBackingCellsOutsideDeclaredExtent(t *testing.T) {
	surface := state.TerminalSurfaceStore{
		TerminalID: "term-live",
		Cols:       2,
		Rows:       2,
		Ready:      true,
		Lines:      []string{"abcdef", "cd", "stale third row"},
	}
	content := buildLiveContentVM(surface, state.TerminalSessionStore{TerminalID: "term-live", Attached: true})
	result := RenderContentViewport(ContentRenderRequest{Rect: Rect{W: 6, H: 4}, Content: content})

	if result.Overflow != (ContentOverflow{}) {
		t.Fatalf("stale backing cells must not create overflow arrows outside a smaller live extent, got %#v", result.Overflow)
	}
	if got := strings.Join(plainContentViewportLines(result.Lines), "\n"); got != "ab····\ncd····\n······\n······" {
		t.Fatalf("declared extent should retain only boundary dots outside the terminal, got %q", got)
	}
}
...
func TestFrameworkPreservesHiddenTerminalLiveCursorThroughViewLayout(t *testing.T) {
	result := NewRenderer(DefaultTheme()).RenderResult(RenderVM{Shell: ShellVM{
		Cursor: Cursor{Visible: true, Row: 0, Col: 0, Shape: CursorShapeBlock},
		Layout: LayoutVM{Viewport: Rect{W: 12, H: 6}, Panels: []PanelVM{{
			ID:           "pane-1",
			Title:        "pane",
			Presentation: PanelPresentationCard,
			...
			Content: ContentVM{
				...
				Extent: ContentExtent{Known: true, Cols: 4, Rows: 2},
				Layout: ContentLayoutVM{Known: true, Mode: "center"},
				Cursor: Cursor{Anchor: true, Row: 1, Col: 2, Shape: CursorShapeBar},
			},
		}}},
	}})

	if result.Cursor != (Cursor{Anchor: true, Row: 2, Col: 5, Shape: CursorShapeBar}) {
		t.Fatalf("hidden live cursor should keep its projected anchor instead of using shell fallback, got %#v", result.Cursor)
	}
	if result.CursorRect != (Rect{X: 6, Y: 3, W: 1, H: 1}) {
		t.Fatalf("hidden live cursor rect should include panel content origin, got %#v", result.CursorRect)
	}
}

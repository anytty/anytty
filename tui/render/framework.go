...
func (renderer Renderer) renderFrameworkCanvas(vm RenderVM) renderedFrameworkCanvas {
	...
	if overlayLayer.Rect.W > 0 && overlayLayer.Rect.H > 0 {
		...
		liveTargets = appendLiveRenderTarget(liveTargets, shell.Overlay.Content)
	}
...
func appendLiveRenderTarget(targets []LiveRenderTarget, content ContentVM) []LiveRenderTarget {
	return appendLiveRenderTargetWithPending(targets, content, false)
}

func appendLiveRenderTargetWithPending(targets []LiveRenderTarget, content ContentVM, allowPending bool) []LiveRenderTarget {
	if content.Kind == ContentTerminalLive && (!content.Pending || allowPending) && content.Error == "" && content.Meta.LiveTerminalID != "" {
	...
	for _, snapshot := range content.Meta.WorkbenchSnapshots {
		// 可见的 manager preview 可能尚无缓存；revision 0 也必须进入 live demand，
		// 才能在不 attach、不接管 resize owner 的前提下启动首次 screen 监听。
		targets = appendLiveRenderTargetWithPending(targets, snapshot.Panel.Content, true)
	}
	return targets
}

...
func runDefaultActionToOwnerBoundary(t *testing.T, root state.Root, invocation actiondomain.Invocation) defaultActionExecution {
	...
	terminal := &testkit.FakeTerminalService{
		AttachResult: port.TerminalAttachResult{
			EndpointID: "west", TerminalID: "term-1", Channel: 11,
			OwnerSurfaceID: "surface-1", OwnerViewID: state.TerminalPaneViewID(state.DefaultPaneID),
		},
...
func terminalServiceCallCount(service *testkit.FakeTerminalService) int {
	return len(service.Attaches) + len(service.Detaches) + len(service.Lists) + len(service.Creates) +
		...
		len(service.Edits) + len(service.TagEdits) + len(service.Inputs) + len(service.Resizes) + len(service.ResizeLocks) + len(service.Surfaces)
}
...
func assertDefaultActionServiceOwner(t *testing.T, invocation actiondomain.Invocation, execution defaultActionExecution) {
	...
	case "copy.copy_selection", "copy.accept":
		if len(execution.core.CopyRequests) != 0 || len(execution.clipboard.Writes) != 1 || len(execution.clipboardStorage.Saves) != 1 {
			t.Fatalf("visible selection must copy locally and persist exactly once: backend=%d writes=%d saves=%d messages=%v", len(execution.core.CopyRequests), len(execution.clipboard.Writes), len(execution.clipboardStorage.Saves), execution.messageTypes)
		}
		expectedReleases := 0
		if id == "copy.accept" {
			expectedReleases = 1
	...
	if !reflect.DeepEqual(gotMutations, wantMutations) {
		t.Fatalf("terminal mutation operation/TerminalRef vector mismatch: got=%#v want=%#v resizes=%#v views=%#v messages=%v", gotMutations, wantMutations, terminal.Resizes, execution.root.TerminalViews.Views, execution.messageTypes)
	}
...
func terminalMutationVectorFromService(terminal *testkit.FakeTerminalService) terminalMutationVector {
	...
	for _, request := range terminal.ResizeLocks {
		vector.ResizeLocks = append(vector.ResizeLocks, state.NewTerminalRef(request.EndpointID, request.TerminalID))
	}
	return vector
}
...
func expectedDefaultActionTerminalMutations(invocation actiondomain.Invocation) terminalMutationVector {
	...
	case "panel.restart":
		return terminalMutationVector{Attaches: []state.TerminalRef{term1}, Restarts: []state.TerminalRef{term1}, Resizes: resizeTerm1}
	case "panel.kill":
	...
	case "panel.size_lock":
		return terminalMutationVector{ResizeLocks: []state.TerminalRef{term1}}
...
func defaultActionExecutionRoot(t *testing.T, id actiondomain.ID) state.Root {
	...
	var sessionStamp *apipb.EndpointSessionStamp
	if strings.HasSuffix(string(id), "take_owner") {
		sessionStamp = &apipb.EndpointSessionStamp{EndpointId: "west", RouteId: "local", Generation: 1}
		ownerBinding, _ := root.TerminalViews.PaneBinding(state.DefaultPaneID)
		ownerBinding.Session = sessionStamp
		root.TerminalViews.Views[ownerBinding.ViewID] = ownerBinding
	}
	...
	if strings.HasPrefix(string(id), "floating.") || strings.HasPrefix(string(id), "floating_overview.") {
		...
		if sessionStamp != nil {
			floatingBinding, _ := root.TerminalViews.FloatingBinding("floating-9")
			floatingBinding.Session = sessionStamp
			root.TerminalViews.Views[floatingBinding.ViewID] = floatingBinding
		}
	}
	switch {

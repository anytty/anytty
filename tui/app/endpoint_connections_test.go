...
func TestConnectionsWatchAppliesExternalToggleAndPreservesRuntimeTruth(t *testing.T) {
	root := connectionsTestRoot()
	studio, _ := root.Endpoints.Endpoint("studio")
	studio.Label = "Renamed Studio"
	studio.Enabled = false
	projection := (state.EndpointStore{}).Upsert(studio)

	disabled, effects := NewEndpointConnectionsReducer(LiveDeps{})(root, ConnectionsWatchUpdateMsg{
		Update: port.EndpointConnectionUpdate{Store: projection},
	})
	studio, _ = disabled.Endpoints.Endpoint("studio")
	if studio.Enabled || studio.Label != "Renamed Studio" || studio.ActiveRouteID != "cloud" || studio.ConnectionGeneration != 9 || studio.ObservedPath != "single_relay" {
		t.Fatalf("external disable projection = %#v", studio)
	}
	if len(effects) != 0 {
		t.Fatalf("external disable scheduled effects = %#v", effects)
	}

	studio.Enabled = true
	enabledProjection := (state.EndpointStore{}).Upsert(studio)
	enabled, effects := NewEndpointConnectionsReducer(LiveDeps{})(disabled, ConnectionsWatchUpdateMsg{
		Update: port.EndpointConnectionUpdate{Store: enabledProjection},
	})
	if item, _ := enabled.Endpoints.Endpoint("studio"); !item.Enabled {
		t.Fatalf("external enable projection = %#v", item)
	}
	if len(effects) != 1 {
		t.Fatalf("external enable effects = %#v", effects)
	}
	request, ok := effects[0].(FuncEffect).Run(context.Background()).(TerminalPoolListRequestMsg)
	if !ok || request.EndpointID != "studio" || !request.Refresh {
		t.Fatalf("external enable refresh = %#v", request)
	}
}

func TestConnectionsWatchKeepsLastValidProjectionOnRegistryError(t *testing.T) {
	root := connectionsTestRoot()
	next, effects := NewEndpointConnectionsReducer(LiveDeps{})(root, ConnectionsWatchUpdateMsg{
		Update: port.EndpointConnectionUpdate{Err: errors.New("parse endpoint registry")},
	})
	studio, ok := next.Endpoints.Endpoint("studio")
	if !ok || studio.ActiveRouteID != "cloud" || studio.ConnectionGeneration != 9 {
		t.Fatalf("registry error discarded last valid projection: %#v ok=%t", studio, ok)
	}
	if len(effects) != 0 || len(next.Shell.Toasts) == 0 || next.Shell.Toasts[len(next.Shell.Toasts)-1].Severity != state.ToastWarning {
		t.Fatalf("registry error effects=%#v toasts=%#v", effects, next.Shell.Toasts)
	}
}

func TestConnectionsWatchRequestUsesCancelableStream(t *testing.T) {
	service := &watchableEndpointConnectionService{updates: make(chan port.EndpointConnectionUpdate, 1)}
	_, effects := NewEndpointConnectionsReducer(LiveDeps{EndpointConnections: service})(connectionsTestRoot(), ConnectionsWatchRequestMsg{})
	if len(effects) != 1 {
		t.Fatalf("registry watch effects = %#v", effects)
	}
	stream, ok := effects[0].(StreamEffect)
	if !ok || stream.Token != connectionsRegistryWatchToken {
		t.Fatalf("registry watch effect = %#v", effects[0])
	}
	...
	service.updates <- port.EndpointConnectionUpdate{Store: connectionsTestRoot().Endpoints}
	select {
	case msg := <-posted:
		if _, ok := msg.(ConnectionsWatchUpdateMsg); !ok {
			t.Fatalf("registry watch posted %T", msg)
		}
	case <-time.After(time.Second):
		t.Fatal("registry watch did not post an update")
	}
	cancel()
	select {
	case <-done:
	case <-time.After(time.Second):
		t.Fatal("registry watch stream did not stop after cancellation")
	}
}

func TestLiveRuntimeStartsConnectionRegistryWatch(t *testing.T) {
	service := &watchableEndpointConnectionService{updates: make(chan port.EndpointConnectionUpdate)}
	runner := &recordingRuntimeEffectRunner{}
	runtime := NewLiveRuntime(connectionsTestRoot(), nil, runner, LiveDeps{EndpointConnections: service})
	if err := runtime.Drain(context.Background()); err != nil {
		t.Fatal(err)
	}
	for _, effect := range runner.Effects {
		if stream, ok := effect.(StreamEffect); ok && stream.Token == connectionsRegistryWatchToken {
			return
		}
	}
	t.Fatalf("live runtime did not start registry watch: %#v", runner.Effects)
}
...
type watchableEndpointConnectionService struct {
	updates chan port.EndpointConnectionUpdate
}

func (service *watchableEndpointConnectionService) LoadConnections(context.Context) (state.EndpointStore, error) {
	return state.EndpointStore{}, nil
}

func (service *watchableEndpointConnectionService) SampleConnection(context.Context, state.EndpointID) (state.EndpointConnectionSnapshot, bool, error) {
	return state.EndpointConnectionSnapshot{}, false, nil
}

func (service *watchableEndpointConnectionService) SetEndpointEnabled(context.Context, state.EndpointID, bool) (state.EndpointStore, error) {
	return state.EndpointStore{}, nil
}

func (service *watchableEndpointConnectionService) ApplyConnectionSettings(context.Context, state.EndpointID, state.EndpointConnectionPolicy, map[string]*int) (state.EndpointStore, error) {
	return state.EndpointStore{}, nil
}

func (service *watchableEndpointConnectionService) WatchConnections(context.Context) (<-chan port.EndpointConnectionUpdate, error) {
	return service.updates, nil
}

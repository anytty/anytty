...
func TestLiveAttachmentProjectionIsSoleTerminalSizeLockTruth(t *testing.T) {
	liveReducer := newLiveReducerPrepared(LiveDeps{})
	resizeReducer := ComposeReducers(liveReducer, NewTerminalLayoutResizeReducer())
	root := state.Root{
		Shell: state.DefaultShell().
			SetPanelPresentation(state.PanelPresentationSplitLine).
			FocusPane(state.PaneCommandTarget{PaneID: state.DefaultPaneID}),
	...
	ownerBinding, _ := root.TerminalViews.PaneBinding(state.DefaultPaneID)
	ownerBinding.OwnerSurfaceID = ownerBinding.SurfaceID
	ownerBinding.OwnerViewID = ownerBinding.ViewID
	root.TerminalViews.Views[ownerBinding.ViewID] = ownerBinding

	metadata, effects := liveReducer(root, LiveEventMsg{Event: port.TerminalLiveEvent{TerminalID: "term-1", Metadata: true, Tags: map[string]string{"anytty.size_lock": "lock"}}})
	if _, ok := liveResizeMsgFromEffects(effects); ok {
		t.Fatalf("metadata tag must not resize PTY, effects=%#v", effects)
	}
	owner, _ := metadata.TerminalViews.PaneBinding(state.DefaultPaneID)
	follower, _ := metadata.TerminalViews.PaneBinding("pane-2")
	if owner.SizeLocked || !owner.CanResize || follower.SizeLocked || follower.CanResize {
		t.Fatalf("metadata tag must not change resize control, owner=%#v follower=%#v", owner, follower)
	}
	if metadata.TerminalPool.Items[0].Tags["anytty.size_lock"] != "lock" {
		t.Fatalf("metadata event should still update terminal pool tags, pool=%#v", metadata.TerminalPool)
	}

	locked, effects := liveReducer(metadata, LiveEventMsg{Event: port.TerminalLiveEvent{
		TerminalID: "term-1", Metadata: true, AttachmentProjection: true, AttachmentCount: 2,
		SizeLocked: true, OwnerSurfaceID: "surface", OwnerViewID: "view-1", ResizeEpoch: 1,
	}})
	if _, ok := liveResizeMsgFromEffects(effects); ok {
		t.Fatalf("daemon lock projection must not resize PTY, effects=%#v", effects)
	}
	owner, _ = locked.TerminalViews.PaneBinding(state.DefaultPaneID)
	follower, _ = locked.TerminalViews.PaneBinding("pane-2")
	if !owner.SizeLocked || owner.CanResize || !follower.SizeLocked || follower.CanResize {
		t.Fatalf("daemon lock should broadcast to all terminal views, owner=%#v follower=%#v", owner, follower)
	}

	unlocked, effects := resizeReducer(locked, LiveEventMsg{Event: port.TerminalLiveEvent{
		TerminalID: "term-1", Metadata: true, AttachmentProjection: true, AttachmentCount: 2,
		OwnerSurfaceID: "surface", OwnerViewID: "view-1", ResizeEpoch: 2,
	}})
	msg, ok := liveResizeMsgFromEffects(effects)
	if !ok {
		t.Fatalf("daemon unlock should trigger owner resize when panel and terminal size diverged, effects=%#v", effects)
	}
	owner, _ = unlocked.TerminalViews.PaneBinding(state.DefaultPaneID)
	follower, _ = unlocked.TerminalViews.PaneBinding("pane-2")
	if owner.SizeLocked || !owner.CanResize || follower.SizeLocked || follower.CanResize {
		t.Fatalf("daemon unlock should broadcast unlocked projection, owner=%#v follower=%#v", owner, follower)
	}
...
func TestTerminalSizeLockBlocksAttachResultResizeToSplitPane(t *testing.T) {
	...
	next, effects := reducer(root, TerminalPoolAttachResultMsg{
		...
		Result: port.TerminalAttachResult{
			TerminalID: "term-1", Channel: 8, Cols: 60, Rows: 14,
			ResizePolicy: state.TerminalResizeRoleOwner, SurfaceID: "surface", ViewID: state.TerminalPaneViewID("pane-2"),
			SizeLocked: true, ControlReason: "size_locked", OwnerSurfaceID: "surface", OwnerViewID: state.TerminalPaneViewID("pane-2"), ResizeEpoch: 1,
		},
	})
	if _, ok := liveResizeMsgFromEffects(effects); ok {
...
func TestQueuedViewResizeRechecksOwnerAndSizeLockBeforeIO(t *testing.T) {
	for _, test := range []struct {
		name   string
		mutate func(state.TerminalViewStore) state.TerminalViewStore
	}{
		{
			name: "owner transferred",
			mutate: func(store state.TerminalViewStore) state.TerminalViewStore {
				return store.TransferPaneResizeOwner("pane-2")
			},
		},
		{
			name: "terminal size locked",
			mutate: func(store state.TerminalViewStore) state.TerminalViewStore {
				return store.ApplyTerminalSizeLock("term-1", true)
			},
		},
	} {
		t.Run(test.name, func(t *testing.T) {
			terminal := &testkit.FakeTerminalService{}
			reducer := newLiveReducerPrepared(LiveDeps{Terminal: terminal})
			root := state.Root{Session: state.TerminalSessionStore{TerminalID: "term-2", Channel: 9, Attached: true}}
			root.TerminalViews = root.TerminalViews.BindPane(state.NewPaneTerminalView("pane-1", "term-1", 7, 80, 24, state.TerminalResizeRoleOwner, "surface-1", "view-1", true))
			root.TerminalViews = root.TerminalViews.BindPane(state.NewPaneTerminalView("pane-2", "term-1", 8, 40, 12, state.TerminalResizeRoleFollower, "surface-2", "view-2", false))
			var decision state.TerminalViewResizeDecision
			root.TerminalViews, decision = root.TerminalViews.RequestViewResize("view-1", 100, 30)
			if !decision.Allowed || !decision.Changed {
				t.Fatalf("expected initial owner resize decision, decision=%#v", decision)
			}
			root.TerminalViews = test.mutate(root.TerminalViews)

			next, effects := reducer(root, LiveResizeMsg{TerminalID: "term-1", Cols: 100, Rows: 30, Seq: decision.Seq, ViewID: "view-1"})
			if len(effects) != 0 || len(terminal.Resizes) != 0 {
				t.Fatalf("stale queued resize must be dropped before terminal IO, effects=%#v requests=%#v", effects, terminal.Resizes)
			}
			if next.Session.TerminalID != "term-2" {
				t.Fatalf("dropped background resize changed active session, session=%#v", next.Session)
			}
		})
	}
}
...
func TestActiveFloatingResizeCommandResizesAttachedTerminalContentRect(t *testing.T) {
	...
	if err := postPreparedTerminalPoolAttachResult(runtime, TerminalPoolAttachResultMsg{
		...
		Result: port.TerminalAttachResult{
			TerminalID: "term-float", Channel: 5, Cols: 28, Rows: 6,
			ResizePolicy: state.TerminalResizeRoleOwner, CanResize: true,
			OwnerSurfaceID: DefaultRuntimeSurfaceID, OwnerViewID: state.TerminalFloatingViewID("floating-1"),
		},
	}); err != nil {
		t.Fatalf("post floating attach result: %v", err)
	}
...
func TestRightStackDividerResizesBothVisibleTerminalOwners(t *testing.T) {
	shell := state.DefaultShell().
		SetPanelPresentation(state.PanelPresentationSplitLine).
		SplitActivePane(state.PaneState{ID: "pane-right", Title: "right", Kind: state.PaneTerminalLive}, state.SplitDirectionVertical).
		SplitActivePane(state.PaneState{ID: "pane-right-bottom", Title: "right bottom", Kind: state.PaneTerminalLive}, state.SplitDirectionHorizontal)
	root := state.Root{
		Viewport: state.ViewportStore{Valid: true, Cols: 140, Rows: 36},
		Shell: shell.
			BindPaneTerminal(state.PaneCommandTarget{PaneID: state.DefaultPaneID}, "term-left").
			BindPaneTerminal(state.PaneCommandTarget{PaneID: "pane-right"}, "term-right-top").
			BindPaneTerminal(state.PaneCommandTarget{PaneID: "pane-right-bottom"}, "term-right-bottom"),
		Session: state.TerminalSessionStore{
	...
	plan, ok := terminalLayoutPlan(root, render.Rect{})
	if !ok || len(plan.Panels) != 3 {
		t.Fatalf("expected 1:2 terminal layout, plan=%#v", plan)
	}
	terminalForPane := map[string]string{
		state.DefaultPaneID: "term-left",
		"pane-right":        "term-right-top",
		"pane-right-bottom": "term-right-bottom",
	}
	channel := uint16(7)
	for _, panel := range plan.Panels {
		terminalID := terminalForPane[panel.Panel.ID]
		root.TerminalViews = root.TerminalViews.BindPane(state.NewPaneTerminalView(
			panel.Panel.ID, terminalID, channel, panel.ContentRect.W, panel.ContentRect.H,
			state.TerminalResizeRoleOwner, "surface-"+terminalID, state.TerminalPaneViewID(panel.Panel.ID), true,
		))
		channel++
	}
	leftBefore, _ := root.TerminalViews.PaneBinding(state.DefaultPaneID)
	root.Shell = root.Shell.ResizePane(state.PaneCommandTarget{PaneID: "pane-right"}, state.PaneResizeDown, 5)

	next, effects := NewTerminalLayoutResizeReducer()(root, ShellPaneCommandMsg{Command: state.PaneCommand{
		Action:          state.PaneCommandResize,
		Target:          state.PaneCommandTarget{PaneID: "pane-right"},
		ResizeDirection: state.PaneResizeDown,
		Delta:           5,
	}})
	if len(effects) != 2 {
		t.Fatalf("right divider must resize both right terminal owners, effects=%#v", effects)
	}
	resized := make(map[string]LiveResizeMsg, len(effects))
	for _, effect := range effects {
		msg, ok := effect.(FuncEffect).Run(context.Background()).(LiveResizeMsg)
		if !ok {
			t.Fatalf("expected live resize message, got %#v", effect)
		}
		resized[msg.TerminalID] = msg
	}
	if _, found := resized["term-left"]; found {
		t.Fatalf("unchanged left pane must not be resized, messages=%#v", resized)
	}
	if resized["term-right-top"].Rows == 0 || resized["term-right-bottom"].Rows == 0 {
		t.Fatalf("both right terminal owners need new nonzero heights, messages=%#v", resized)
	}
	leftAfter, _ := next.TerminalViews.PaneBinding(state.DefaultPaneID)
	if leftAfter.DesiredCols != leftBefore.DesiredCols || leftAfter.DesiredRows != leftBefore.DesiredRows || leftAfter.RequestSeq != leftBefore.RequestSeq {
		t.Fatalf("right divider changed left terminal geometry, before=%#v after=%#v", leftBefore, leftAfter)
	}
}

func TestRightStackDividerDragResizesPassiveOwnersWithoutPollutingActiveTerminal(t *testing.T) {
	shell := state.DefaultShell().
		SetPanelPresentation(state.PanelPresentationSplitLine).
		SplitActivePane(state.PaneState{ID: "pane-right", Title: "right", Kind: state.PaneTerminalLive}, state.SplitDirectionVertical).
		SplitActivePane(state.PaneState{ID: "pane-right-bottom", Title: "right bottom", Kind: state.PaneTerminalLive}, state.SplitDirectionHorizontal).
	...
	channel := uint16(7)
	attachmentSession := &apipb.EndpointSessionStamp{EndpointId: "local", RouteId: "route-1", Generation: 1}
	for _, panel := range plan.Panels {
		terminalID := terminalForPane[panel.Panel.ID]
		binding := state.NewPaneTerminalView(
			panel.Panel.ID, terminalID, channel, panel.ContentRect.W, panel.ContentRect.H,
			state.TerminalResizeRoleOwner, "surface-"+terminalID, state.TerminalPaneViewID(panel.Panel.ID), true,
		)
		binding.Session = attachmentSession
		root.TerminalViews = root.TerminalViews.BindPane(binding)
		ref := state.NewTerminalRef("", terminalID)
		if terminalID == "term-left" {
			root.Session = root.Session.Resize(panel.ContentRect.W, panel.ContentRect.H)
			root.Surface = root.Surface.AttachRef(ref, panel.ContentRect.W, panel.ContentRect.H)
		} else {
			root.Surface = root.Surface.CacheAttachRef(ref, panel.ContentRect.W, panel.ContentRect.H)
		}
		channel++
	}
	activeCols, activeRows := root.Session.Cols, root.Session.Rows
	terminal := &testkit.FakeTerminalService{}
	...
	resizeRegion := framePaneResizeRegion(t, lastRuntimeFrame(t, host), "pane-right", state.PaneResizeDown)
	start := mouseEventAt(resizeRegion.Rect)
	start.Col = resizeRegion.Rect.X + resizeRegion.Rect.W/2 + 1
	...
	if !runtime.mouseDrag.Active || runtime.mouseDrag.PaneID != "pane-right" || runtime.mouseDrag.Direction != state.PaneResizeDown {
		t.Fatalf("expected right stack divider drag state, drag=%#v", runtime.mouseDrag)
	}
	...
	if err := runtime.Drain(context.Background()); err != nil {
		t.Fatalf("drain resize results: %v", err)
	}
	afterRects := paneLayoutRects(runtime.State())
	if afterRects["pane-right"] == beforeRects["pane-right"] && afterRects["pane-right-bottom"] == beforeRects["pane-right-bottom"] {
		t.Fatalf("right stack divider drag did not change split geometry, drag=%#v", runtime.mouseDrag)
	}

	resized := make(map[string]port.TerminalResizeRequest, len(terminal.Resizes))
	for _, request := range terminal.Resizes {
		resized[request.TerminalID] = request
	}
	if len(resized) != 2 || resized["term-right-top"].Rows == 0 || resized["term-right-bottom"].Rows == 0 {
		t.Fatalf("drag must resize both passive right-side owners, requests=%#v views=%#v", terminal.Resizes, runtime.State().TerminalViews.Views)
	}
	if _, found := resized["term-left"]; found {
		t.Fatalf("unchanged active terminal must not be resized, requests=%#v", terminal.Resizes)
	}
	next := runtime.State()
	if next.Session.TerminalID != "term-left" || next.Session.Cols != activeCols || next.Session.Rows != activeRows {
		t.Fatalf("passive resize result polluted active session, session=%#v", next.Session)
	}
	if next.Surface.TerminalID != "term-left" || next.Surface.Cols != activeCols || next.Surface.Rows != activeRows || next.Surface.ResizeBoundary.Active {
		t.Fatalf("passive resize result polluted active surface, surface=%#v", next.Surface)
	}
	for _, terminalID := range []string{"term-right-top", "term-right-bottom"} {
		request := resized[terminalID]
		surface := next.Surface.SurfaceForTerminalRef(state.NewTerminalRef("", terminalID))
		if surface.Cols != request.Cols || surface.Rows != request.Rows {
			binding, _ := next.TerminalViews.Views[request.ViewID]
			t.Fatalf("passive surface %s did not cache resized geometry, surface=%#v request=%#v binding=%#v", terminalID, surface, request, binding)
		}
	}
}
...
func TestCloseResizeOwnerWaitsForDaemonPromotionBeforeResize(t *testing.T) {
	terminal := &testkit.FakeTerminalService{}
	host := NewFakeTerminalHost(16)
	host.SetSize(122, 28)
	shell := state.DefaultShell().SetPanelPresentation(state.PanelPresentationSplitLine)
	...
	if len(terminal.Resizes) != 0 {
		t.Fatalf("TUI must not resize before daemon promotes a new owner, got %#v", terminal.Resizes)
	}
	...
	binding, ok := runtime.State().TerminalViews.PaneBinding("pane-2")
	if !ok || binding.ResizeRole != state.TerminalResizeRoleFollower || binding.CanResize || binding.ResizePending {
		t.Fatalf("remaining pane must stay follower before daemon event, binding=%#v ok=%v", binding, ok)
	}
	if err := runtime.Post(LiveEventMsg{Event: port.TerminalLiveEvent{
		TerminalID: "term-1", Metadata: true, AttachmentProjection: true, AttachmentCount: 1,
		OwnerSurfaceID: "surface", OwnerViewID: "view-2", ResizeEpoch: 1,
	}}); err != nil {
		t.Fatalf("post daemon owner projection: %v", err)
	}
	if err := runtime.Drain(context.Background()); err != nil {
		t.Fatalf("drain daemon owner projection: %v", err)
	}
	if len(terminal.Resizes) != 1 {
		t.Fatalf("daemon-promoted follower should resize exactly once, got %#v", terminal.Resizes)
	}
	if got := terminal.Resizes[0]; got.TerminalID != "term-1" || got.Channel != 8 || got.ViewID != "view-2" || got.ResizePolicy != state.TerminalResizeRoleOwner || got.Cols != 120 || got.Rows != 24 {
		t.Fatalf("daemon-promoted owner resize should use its content rect and channel, got %#v", got)
	}
	binding, ok = runtime.State().TerminalViews.PaneBinding("pane-2")
	if !ok || binding.ResizeRole != state.TerminalResizeRoleOwner || !binding.CanResize || binding.ResizePending || binding.RequestSeq != 1 {
		t.Fatalf("remaining pane should own resize after daemon projection, binding=%#v ok=%v", binding, ok)
	}
}

...
func (runtime *AppRuntime) consumeTakeResizeOwnerDoubleClick(region render.HitRegion, event input.InputEvent) bool {
	...
	if runtime.lastMouseAction == current && !runtime.lastMouseActionAt.IsZero() && now.Sub(runtime.lastMouseActionAt) <= ownerConfirmDelay {
		runtime.lastMouseAction = mouseActionClickState{}
		runtime.lastMouseActionAt = time.Time{}
		return true
	}
	runtime.lastMouseAction = current
	runtime.lastMouseActionAt = now
	return false
}

...
func reduceTerminalPoolAttachResult(root state.Root, msg TerminalPoolAttachResultMsg, deps LiveDeps) (state.Root, []Effect) {
	...
	effects = append(effects, terminalEventWatchRequestEffects(result.EndpointID, deps)...)
...
func reduceTerminalSizeLockToggleRequest(root state.Root, deps LiveDeps) (state.Root, []Effect) {
	...
	locked := !target.SizeLocked
	var operationID string
	root.TerminalViews, operationID = root.TerminalViews.NextTerminalOperation("resize-lock", target.ViewID)
	return root, []Effect{FuncEffect{Run: func(ctx context.Context) Msg {
		result, err := deps.Terminal.ResizeLock(ctx, port.TerminalResizeLockRequest{
			EndpointID: target.EndpointID, TerminalID: target.TerminalID, Channel: target.Channel,
			Locked: locked, SurfaceID: target.SurfaceID, ViewID: target.ViewID,
			Session: target.Session, OperationID: operationID,
		})
		return TerminalSizeLockToggleResultMsg{EndpointID: target.EndpointID, TerminalID: target.TerminalID, Locked: result.SizeLocked, Result: result, Err: err}
	}}}
}
...
func reduceTerminalSizeLockToggleResult(root state.Root, msg TerminalSizeLockToggleResultMsg) (state.Root, []Effect) {
	...
	if hasResizeControlResult(msg.Result) {
		root.TerminalViews = root.TerminalViews.ApplyTerminalRefResizeControl(ref, resizeControlProjectionFromResult(msg.Result))
	} else {
...
func activeTerminalSizeLockTarget(root state.Root) (terminalSizeLockTarget, bool) {
	if binding, ok := activeTerminalViewBinding(root); ok && binding.TerminalID != "" {
		if binding.HasProjectedResizeOwner() {
			return terminalSizeLockTarget{
				EndpointID: binding.EndpointID, TerminalID: binding.TerminalID, Channel: binding.Channel,
				SizeLocked: binding.SizeLocked, SurfaceID: binding.SurfaceID, ViewID: binding.ViewID,
...
func projectTerminalAttachResizeControl(store state.TerminalViewStore, result port.TerminalAttachResult) state.TerminalViewStore {
	return store.ApplyTerminalRefResizeControl(state.NewTerminalRef(result.EndpointID, result.TerminalID), state.TerminalResizeControlProjection{
...
func terminalPoolItemsFromService(items []port.TerminalPoolItem) []state.TerminalPoolItem {
	...
	for i, item := range items {
		out[i] = state.TerminalPoolItem{
			...
			ResourceHistory: terminalResourceHistoryFromService(item.ResourceHistory),
		}
	}
	return out
}

func terminalResourceHistoryFromService(samples []port.TerminalResourceUsage) []state.TerminalResourceUsage {
	if len(samples) == 0 {
		return nil
	}
	out := make([]state.TerminalResourceUsage, len(samples))
	for index, sample := range samples {
		out[index] = state.TerminalResourceUsage{
			PID: sample.PID, CPUPercentX100: sample.CPUPercentX100, MemoryBytes: sample.MemoryBytes, SampledAt: sample.SampledAt,
		}
	}
	return out
}

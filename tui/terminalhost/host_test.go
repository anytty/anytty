...
func TestFrameSinkParksHiddenCursorAtLastVisiblePosition(t *testing.T) {
	var output bytes.Buffer
	sink := NewFrameSink(&output)
	visible := render.Frame{
		Lines:      []string{"one", "two"},
		Cursor:     render.Cursor{Visible: true, Shape: render.CursorShapeBar},
		CursorRect: render.Rect{X: 4, Y: 2, W: 1, H: 1},
	}
	if err := sink.WriteFrame(visible); err != nil {
		t.Fatalf("write visible frame: %v", err)
	}

	output.Reset()
	hidden := visible
	hidden.Cursor = render.Cursor{Anchor: true, Shape: render.CursorShapeBar}
	hidden.CursorRect = render.Rect{X: 9, Y: 7, W: 1, H: 1}
	if err := sink.WriteFrame(hidden); err != nil {
		t.Fatalf("write hidden frame: %v", err)
	}
	got := output.String()
	if !strings.Contains(got, hideCursor+cursorShapeBar+cursorPosition(3, 5)) {
		t.Fatalf("hidden cursor should stay at the last visible position, got %q", got)
	}
	if strings.Contains(got, cursorPosition(8, 10)) {
		t.Fatalf("hidden cursor must ignore the application's transient hidden position, got %q", got)
	}
}

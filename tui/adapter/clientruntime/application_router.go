...
func (router *EndpointApplicationRouter) ResizeLock(ctx context.Context, req port.TerminalResizeLockRequest) (port.TerminalResizeResult, error) {
	adapter, err := router.terminal(ctx, req.EndpointID)
	if err != nil {
		return port.TerminalResizeResult{}, err
	}
	return adapter.ResizeLock(ctx, req)
}
...
func (router *EndpointApplicationRouter) WatchTerminalEvents(ctx context.Context, endpointID state.EndpointID) (<-chan port.TerminalLiveEvent, error) {
	adapter, err := router.terminal(ctx, endpointID)
	if err != nil {
		return nil, err
	}
	return adapter.WatchTerminalEvents(ctx, endpointID)
}
...
var _ port.TerminalEventSource = (*EndpointApplicationRouter)(nil)

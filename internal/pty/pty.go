...
type ConPty interface {
	...
	// ClosePseudoConsole closes the host while leaving the pipes open for drain.
	ClosePseudoConsole() error

	// ClosePipes closes the caller-owned input and output pipes.
	ClosePipes() error
}

...
func TestDaemonRuntimeRecordKeepsDirectListenerForPairing(t *testing.T) {
	t.Setenv("ANYTTY_DIRECT_LISTEN", "0.0.0.0:45123")
	runtimeDir := t.TempDir()
	socketPath := filepath.Join(runtimeDir, "anytty.sock")
	release, err := acquireDaemonRuntimeRecord(socketPath, filepath.Join(runtimeDir, "anytty.log"), filepath.Join(runtimeDir, "config.json"))
	if err != nil {
		t.Fatal(err)
	}
	defer release()
	...
	if got := runningDaemonDirectListen(socketPath); got != record.DirectListen {

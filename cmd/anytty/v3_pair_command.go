...
func v3PairCreateCommand(socket *string, logFile *string) *cobra.Command {
	...
	command.Flags().StringArrayVar(&directAddresses, "direct-address", nil, "published Direct HOST:PORT used for both setup and ICE TCP (repeatable)")
...
func renderV3PairingPreview(output io.Writer, payload []byte, expiresAt time.Time) error {
	...
	for _, route := range offer.GetRoutes() {
		if direct := route.GetDirectWebrtcTcp(); direct != nil {
			details := directPairingPreviewDetails(direct)
...
func directPairingPreviewDetails(direct *remoteauthpb.PairingDirectRouteSeed) string {
	signaling := direct.GetSignalingAddress()
	iceTCP := direct.GetIceTcpAddress()
	if signaling == iceTCP {
		return fmt.Sprintf("address=%s (shared signaling + ICE TCP)", signaling)
	}
	return fmt.Sprintf("signaling=%s, ice-tcp=%s", signaling, iceTCP)
}

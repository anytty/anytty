...
func terminalInfoView(endpointID endpointdomain.EndpointID, item *apipb.TerminalInfo) terminalView {
	...
	return terminalView{
		...
		CreatedAt: formatTerminalUnixNano(item.GetCreatedAtUnixNano()), ForegroundProcess: item.GetForegroundProcess(), LastOutputAt: formatTerminalUnixNano(item.GetLastOutputAtUnixNano()), ExitCode: exitCode, ExitedAt: formatTerminalUnixNano(item.GetExitedAtUnixNano()),
	}
}
...
func writeTerminalDetail(writer io.Writer, view terminalView) error {
	return writeCLIFields(writer,
		...
		cliField{Label: "Foreground", Value: view.ForegroundProcess},
		cliField{Label: "Last output", Value: view.LastOutputAt},

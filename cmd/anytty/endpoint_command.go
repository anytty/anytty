...
type routeEditFlags struct {
	...
	priority                                                                  int
	manualOnly                                                                bool
	hostKeyFingerprints, directAddresses, signalingAddresses, iceTCPAddresses []string
...
func routeFromFlags(kind endpointdomain.RouteKind, flags routeEditFlags) endpointdomain.AccessRoute {
	...
	if len(flags.directAddresses) > 0 {
		route.SignalingAddresses = append([]string(nil), flags.directAddresses...)
		route.ICETCPAddresses = append([]string(nil), flags.directAddresses...)
	}
	if flags.priority >= 0 {
...
func applyRouteFlagChanges(command *cobra.Command, route *endpointdomain.AccessRoute, flags routeEditFlags) {
	...
	if command.Flags().Changed("direct-address") {
		route.SignalingAddresses = append([]string(nil), flags.directAddresses...)
		route.ICETCPAddresses = append([]string(nil), flags.directAddresses...)
	}

...
func TestClientEndpointShareBundleIsConfigOnlyAndProducesDiff(t *testing.T) {
	...
	target := Endpoint{
		...
		Platform:       "linux", ConnectMode: ConnectAuto, Enabled: true, SelectionPolicy: SelectionPolicy{HedgeDelayConfigured: true, HedgeDelay: 250 * time.Millisecond},
	...
	if candidate.Platform != "linux" {
		t.Fatalf("shared platform = %q", candidate.Platform)
	}

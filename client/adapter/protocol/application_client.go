...
func (client *ApplicationClient) Err() error {
	...
	return classifyTransportTerminalError(client.Client.Err())
}

func classifyTransportTerminalError(err error) error {
	if err == nil || errors.Is(err, io.EOF) || errors.Is(err, context.Canceled) {
		return err
	}
	var runtimeErr *clientruntime.Error
	var peerErr *internalprotocol.PeerError
	var requestErr *internalprotocol.RequestError
	if errors.As(err, &runtimeErr) || errors.As(err, &peerErr) || errors.As(err, &requestErr) ||
		errors.Is(err, datachannel.ErrReceiveQueueExhausted) || errors.Is(err, wire.ErrFrameTooLarge) {
		return err
	}
	return &clientruntime.Error{
		Code: clientruntime.ErrorUnavailable, Message: "transport connection was interrupted",
		Cause: err, Attempted: true, Retryable: true,
	}
}

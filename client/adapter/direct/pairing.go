...
func (connector *PairingConnector) Redeem(ctx context.Context, request clientruntime.AttemptRequest, pairing remoteauth.ClientPairingRequest) (remoteauth.PairingExchangeResult, error) {
	...
	reportTiming := func(stage string) {
		now := time.Now()
		log.Printf("anytty direct connect generation=%d stage=%s stage_ms=%d total_ms=%d", request.Stamp().Generation, stage, now.Sub(lastAt).Milliseconds(), now.Sub(startedAt).Milliseconds())
		lastAt = now
	}
	opened, err := openDirectPeer(ctx, request, directPeerOptions{
		...
		Random: connector.Random, Now: connector.Now, Phase: connector.Phase, Timing: reportTiming,
	...
	if err != nil {
		return remoteauth.PairingExchangeResult{}, reportDirectFailure(request.Stamp().Generation, directFailurePairingSetup, err)
	}
	result, err := (clientruntime.PairingService{Now: connector.Now}).Redeem(ctx, request, opened, pairing)
	if err != nil {
		return remoteauth.PairingExchangeResult{}, reportDirectFailure(request.Stamp().Generation, directFailurePairingHandshake, err)
	}
	reportTiming("pairing_authenticated")
	return result, nil
}

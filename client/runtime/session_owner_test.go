...
func TestSessionOwnerReplacesGenerationAndRejectsStaleOperations(t *testing.T) {
	...
	if secondLease.Stamp.Generation != firstLease.Stamp.Generation+1 || !firstDialer.session.closed.Load() {
		t.Fatalf("generation first=%#v second=%#v firstClosed=%v", firstLease, secondLease, firstDialer.session.closed.Load())
	}
	...
	if secondDialer.session.closed.Load() {
...
func TestSessionOwnerClosesLateAttemptAfterNewGeneration(t *testing.T) {
	...
	if lateDialer.session == nil || !lateDialer.session.closed.Load() || winner.session.closed.Load() {
		t.Fatalf("lateClosed=%v winnerClosed=%v", lateDialer.session != nil && lateDialer.session.closed.Load(), winner.session.closed.Load())
	}
}
...
func TestSessionOwnerBeginRouteAttemptInvalidatesCurrentSession(t *testing.T) {
	...
	if attempt.Stamp().Generation != lease.Stamp.Generation+1 || !dialer.session.closed.Load() {
		t.Fatalf("lease=%#v attempt=%#v closed=%v", lease, attempt, dialer.session.closed.Load())
	}
...
func TestSessionOwnerFastInvalidationFencesLateReadyAttempt(t *testing.T) {
	owner := NewSessionOwner()
	defer owner.Close()
	target := ownerEndpoint()
	target.Routes["direct"] = endpoint.AccessRoute{ID: "direct", Kind: endpoint.RouteDirectWebRTCTCP, Enabled: true, Source: endpoint.SourceBootstrap, PolicySource: endpoint.SourceUser, SignalingAddresses: []string{"127.0.0.1:41120"}, ICETCPAddresses: []string{"127.0.0.1:41121"}}
	attempts, err := owner.BeginRouteAttempts(target, []endpoint.RouteID{"cloud", "direct"}, ConnectIntentInteractive)
	if err != nil {
		t.Fatal(err)
	}
	readyEvidence := ReadyPeerSessionEvidence{
		Identity: target.DaemonIdentity, IdentityVerified: true, AuthorizationVerified: true, ProtocolVersion: 1,
	}
	current := newOwnerSession(attempts[0].Stamp())
	current.evidence = readyEvidence
	lease, err := owner.AdoptReadyPeerSession(attempts[0], current)
	if err != nil {
		t.Fatal(err)
	}
	cause := errors.New("network generation changed")
	if err := owner.InvalidateSessionFast(lease.Stamp, cause); err != nil {
		t.Fatal(err)
	}

	late := newOwnerSession(attempts[1].Stamp())
	late.evidence = readyEvidence
	if _, err := owner.AdoptReadyPeerSession(attempts[1], late); CodeOf(err) != ErrorStaleSession {
		t.Fatalf("late ready attempt error = %v", err)
	}
	if !late.closed.Load() {
		t.Fatal("late ready attempt remained open after its generation was fenced")
	}
	if _, err := owner.ApplicationSession(lease); CodeOf(err) != ErrorStaleSession {
		t.Fatalf("invalidated lease error = %v", err)
	}
	select {
	case <-current.Done():
	case <-time.After(time.Second):
		t.Fatal("fast invalidation did not close the detached transport")
	}
}

func TestSessionOwnerFastInvalidationDoesNotWaitForTransportClose(t *testing.T) {
	owner := NewSessionOwner()
	defer owner.Close()
	target := ownerEndpoint()
	dialer := &ownerDialer{delayDone: true}
	lease, err := owner.ConnectRoute(context.Background(), target, "cloud", ConnectIntentInteractive, dialer)
	if err != nil {
		t.Fatal(err)
	}
	dialer.session.closeStarted = make(chan struct{})
	dialer.session.closeRelease = make(chan struct{})
	done := make(chan error, 1)
	go func() { done <- owner.InvalidateSessionFast(lease.Stamp, errors.New("network replaced")) }()
	...
	case <-time.After(time.Second):
		t.Fatal("fast invalidation waited for stale transport Close")
	}
	select {
	case <-dialer.session.closeStarted:
	case <-time.After(time.Second):
		t.Fatal("fast invalidation did not start stale transport Close")
	}
	close(dialer.session.closeRelease)
	dialer.session.finishDone()
}

func TestSessionOwnerDisconnectEndpointWaitsForAcquireAndClosesLateWinner(t *testing.T) {
	owner := NewSessionOwner()
	defer owner.Close()
	target := ownerEndpoint()
	started := make(chan struct{})
	...
	if dialer.session == nil || !dialer.session.closed.Load() {
		t.Fatal("explicit endpoint disconnect left the late winner pooled")
	}
...
func TestSessionGenerationAuthorityRevokesSessionOwnedByAnotherHost(t *testing.T) {
	...
	if !firstDialer.session.closed.Load() {
...
func TestSessionOwnerDelayedOldOfflineDoesNotFollowNewReady(t *testing.T) {
	owner := NewSessionOwner()
	defer owner.Close()
	target := ownerEndpoint()
	firstDialer := &ownerDialer{delayDone: true}
	first, err := owner.ConnectRoute(context.Background(), target, "cloud", ConnectIntentInteractive, firstDialer)
	if err != nil {
		t.Fatal(err)
	}
	events, err := owner.WatchEndpoint(context.Background(), target.ID)
	if err != nil {
		t.Fatal(err)
	}
	if event := <-events; event.Phase != EndpointPhaseReady || event.Stamp != first.Stamp {
		t.Fatalf("initial event = %#v", event)
	}
	firstDialer.session.closeStarted = make(chan struct{})
	firstDialer.session.closeRelease = make(chan struct{})
	firstDialer.session.finishDone()
	select {
	case <-firstDialer.session.closeStarted:
	case <-time.After(time.Second):
		t.Fatal("old session cleanup did not reach Close")
	}

	secondDialer := &ownerDialer{}
	second, err := owner.ConnectRoute(context.Background(), target, "cloud", ConnectIntentInteractive, secondDialer)
	if err != nil {
		t.Fatal(err)
	}
	owner.publishEndpointEvent(EndpointEvent{EndpointID: target.ID, Stamp: second.Stamp, Phase: EndpointPhaseReady})
	if event := <-events; event.Phase != EndpointPhaseReady || event.Stamp != second.Stamp {
		t.Fatalf("new ready event = %#v", event)
	}
	close(firstDialer.session.closeRelease)

	select {
	case event := <-events:
		t.Fatalf("old generation published after new Ready: %#v", event)
...
func (session *ownerSession) Close() error {
	if session.closeStarted != nil {
		session.closeStartOnce.Do(func() { close(session.closeStarted) })

...
// RemoteCloudStatus executes the remote_cloud_status application command.
func (session *ApplicationSession) RemoteCloudStatus(ctx context.Context, command *apipb.RemoteCloudStatusCommand) (*apipb.RemoteCloudStatusResult, error) {
	result, err := session.Execute(ctx, &apipb.CommandEnvelope{Command: &apipb.CommandEnvelope_RemoteCloudStatus{RemoteCloudStatus: command}})
	if err != nil {
		return nil, err
	}
	if result.GetRemoteCloudStatus() == nil {
		return nil, missingApplicationResult("remote_cloud_status")
	}
	return result.GetRemoteCloudStatus(), nil
}

// RemoteCloudEnable executes the remote_cloud_enable application command.
func (session *ApplicationSession) RemoteCloudEnable(ctx context.Context, command *apipb.RemoteCloudEnableCommand) (*apipb.RemoteCloudStatusResult, error) {
	result, err := session.Execute(ctx, &apipb.CommandEnvelope{Command: &apipb.CommandEnvelope_RemoteCloudEnable{RemoteCloudEnable: command}})
	if err != nil {
		return nil, err
	}
	if result.GetRemoteCloudStatus() == nil {
...
// RemoteCloudDisable executes the remote_cloud_disable application command.
func (session *ApplicationSession) RemoteCloudDisable(ctx context.Context, command *apipb.RemoteCloudDisableCommand) (*apipb.RemoteCloudStatusResult, error) {
	result, err := session.Execute(ctx, &apipb.CommandEnvelope{Command: &apipb.CommandEnvelope_RemoteCloudDisable{RemoteCloudDisable: command}})
	if err != nil {
		return nil, err
	}
	if result.GetRemoteCloudStatus() == nil {

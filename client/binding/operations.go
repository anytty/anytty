...
func (engine *Engine) EngineCommand(payload []byte) (uint64, error) {
	...
	case *bindingpb.EngineCommand_EndpointDisconnect:
		return engine.startEndpointDisconnect(value.EndpointDisconnect)
	case *bindingpb.EngineCommand_EndpointCloudPresenceGet:
		return engine.startEndpointCloudPresenceGet(value.EndpointCloudPresenceGet)
	default:
		return 0, fmt.Errorf("engine command is required")
	}
}

func (engine *Engine) cloudPresenceHost() (CloudPresenceHost, error) {
	host, ok := engine.host.(CloudPresenceHost)
	if !ok {
		return nil, fmt.Errorf("binding host does not support Cloud presence operations")
	}
	return host, nil
}

func (engine *Engine) startEndpointCloudPresenceGet(request *bindingpb.EndpointCloudPresenceGetRequest) (uint64, error) {
	if request == nil || request.GetRequestId() == "" || request.GetEndpointId() == "" {
		return 0, fmt.Errorf("endpoint Cloud presence request is incomplete")
	}
	host, err := engine.cloudPresenceHost()
	if err != nil {
		return 0, err
	}
	...
	go engine.runEndpointCloudPresenceGet(handle, operationContext, host, proto.Clone(request).(*bindingpb.EndpointCloudPresenceGetRequest))
	return handle, nil
}

func (engine *Engine) endpointDisconnectionHost() (EndpointDisconnectionHost, error) {
	host, ok := engine.host.(EndpointDisconnectionHost)
	if !ok {
		return nil, fmt.Errorf("binding host does not support endpoint disconnection")
	}
	return host, nil
}

func (engine *Engine) startEndpointDisconnect(request *bindingpb.EndpointDisconnectRequest) (uint64, error) {
	if request == nil || request.GetRequestId() == "" || request.GetEndpointId() == "" {
		return 0, fmt.Errorf("endpoint disconnect request is incomplete")
	}
	host, err := engine.endpointDisconnectionHost()
	if err != nil {
		return 0, err
	}
	handle, operationContext, err := engine.startOperation()
...
func (engine *Engine) runEndpointDisconnect(handle uint64, ctx context.Context, host EndpointDisconnectionHost, request *bindingpb.EndpointDisconnectRequest) {
	...
	engine.markOperationDone(handle)
	result := &bindingpb.EndpointDisconnectResult{
		RequestId: request.GetRequestId(), OperationHandle: handle, EndpointId: request.GetEndpointId(),
	}
	if err != nil {
		result.Error = apiError(err)
	}
...
func (engine *Engine) runEndpointCloudPresenceGet(handle uint64, ctx context.Context, host CloudPresenceHost, request *bindingpb.EndpointCloudPresenceGetRequest) {
	result, err := host.GetEndpointCloudPresence(ctx, request)
	if ctx.Err() != nil {
		result, err = nil, ctx.Err()
	}
	engine.markOperationDone(handle)
	if result == nil {
		...
		result = proto.Clone(result).(*bindingpb.EndpointCloudPresenceGetResult)
	}
	result.RequestId, result.OperationHandle, result.EndpointId = request.GetRequestId(), handle, request.GetEndpointId()
	if err != nil {
		result.Error = apiError(err)
	}

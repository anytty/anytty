...
// This file is the layout-program-side display width table. The SDK must stay
// buildable against stdlib + proto + protobuf only (see deps_test.go), so it
// cannot borrow kernel.DisplayWidth/Truncate: the kernel belongs to the host.
// The table is intentionally the same minimal east-asian-width subset the
// host kernel uses, which keeps program-side measurement and host-side layout
// in agreement.

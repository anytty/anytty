...
> 本文是 endpoint 模型、daemon 协议调用序列与 v1 支持矩阵的规格。
...
### 1.1 老 endpoints.yaml（v3，daemon 连接策略）
...
manual 手动）；daemon 侧维护连接状态/租约/重连。`scripts/anytty-dev.sh` 的
...
### 1.2 daemon 协议调用序列（wire v7，`proto/wire` + `proto/apipb`）
...
1. **连接/握手**：`DialContext(socket)` → `TypeHello{Version:7, Client}` →
   daemon 回 `TypeHello{Version:7, Server}`；版本不符或未握手就发请求会被 daemon
...
3. **attach（订阅 surface/revision）**：`TerminalAttachCommand{terminal,
   ...
   size, resize_control}`。`ResourceHandle.opaque_token` 前 2 字节是 daemon 分配的
   stream channel；客户端在该 channel 发 `TypeBootstrapDone` 后 daemon 回
   ...
   modes}`：revision 是 daemon live screen 的权威版本，attach/重连后用它建立画面，
...
| `kind` | `command`（缺省，本地 PTY 跑 argv）或 `daemon`（连接已有 daemon） |
| `label` | picker 展示名，缺省用 `name` |
| `address` | `tcp` 模式必填的 `HOST:PORT`：对端必须是 daemon transport 的透明隧道（`ssh -L TCP→remote socket` 或 socat/`clients/tui/scripts/remote-bridge`） |
| `socket` | `daemon` + `connect_mode=local-unix` 必填的 unix socket 路径（也可以指向 `ssh -L` 转发的本地 socket） |
| `argv`/`cwd`/`env` | command endpoint 的进程参数；daemon endpoint 下是"新建终端"的可选 command/cwd/env（空 argv = daemon 默认 command） |
| `connect_mode` | `local-unix`（daemon 缺省）、`tcp`、`direct-webrtc-tcp`（不做）；未知值配置报错 |

`kind` 在配置层校验：command 必须有 argv；daemon + local-unix 必须有 socket；
daemon + tcp 必须有 address；webrtc 允许配置（picker 仍显示），连接时给可读错误。

### 2.2 picker 展示与分组

- shell 启动收到 HELLO 后，对每个 `kind=daemon` endpoint 发 `endpoint.sync`
  （host 注册并后台连接），daemon 终端清单随后作为 `sources` 到达。
- 配置了任意 daemon endpoint 时 picker 按 endpoint 分组：组头一行
  `  <endpoint>`，组内是 source 行（信息列 `<endpoint> · live/exited/offline/
  connecting`）与配置 endpoint 行（`󰌷 <label>`，信息列 `endpoint · daemon
  <connect_mode>`）；`+ New terminal` 归入 `local` 组。没有 daemon endpoint 时
  保持 v1 平铺像素（兼容规则）。
...
- `terminal.create{endpoint, argv?, cwd?, env?, kind?, socket?, address?, connect_mode?}`：
  daemon endpoint 下 host 先 `TerminalCreate`（空 argv 用 `TerminalDefaults`），
  以 daemon 返回的 terminal id 作为 source id，再 attach。
- `terminal.attach{endpoint, id, fit?, kind?, socket?, address?, connect_mode?}`：
  host 用 kind/socket/address/connect_mode 注册 endpoint 后 attach 已有 daemon 终端。
...
- 已 attach 的终端在重连后**重新 attach 同一 terminal id**、重新拉
  ...
  EOF（终端不因断线被判定退出）。
- `terminal.kill` → daemon `TerminalKill`（槽显示 `[exited N]`）；
  `terminal.remove` → daemon `TerminalRemove`；`Ctrl-E` → daemon
  `TerminalRestart` 后原地重 attach；TUI 退出只 detach。
- daemon 终端与本地 PTY 共用 `runtime.Terminal`/组件/输入路由/scrollback，
  位置透明；resize 失败（owner CAS 冲突/尺寸锁）返回可读错误。

## 3. v1 支持矩阵

| 能力 | command | daemon local-unix | daemon tcp | direct-webrtc-tcp / managed-webrtc |
|---|---|---|---|---|
| 配置 + picker 展示 | ✅ | ✅（分组 + 状态） | ✅（分组 + 状态，`address` 必填） | 配置可写，连接报"不做" |
| list / attach / input | 本地 PTY | ✅ 真实 daemon | ✅ 真实 daemon（`ssh -L`/桥） | ❌ |

...
export default defineConfig(({ command, mode }) => ({
  optimizeDeps: { include: ['@rive-app/webgl2'] },
  plugins: [react(), command === 'build' && mode !== 'test' && fileViewerRenderers({ formats: ['md', 'markdown', 'txt', 'json', 'diff', 'patch', 'pdf', 'doc', 'docx', 'rtf', 'odt', 'xlsx', 'xls', 'csv', 'tsv', 'ods', 'mp4', 'webm', 'm3u8', 'glb', 'gltf', 'obj', 'stl', 'ply', 'fbx', 'dae', '3ds', '3mf', 'amf', 'pcd', 'wrl', 'vrml', 'xyz', 'vtk'], inject: false, copyAssets: true, chunkStrategy: 'none' }), routeFileViewerConsoleToReleaseSink(), assertInitialSourceClosure()],
  build: {
    outDir: 'dist',
    manifest: true,
    minify: 'oxc',
...
function assertInitialSourceClosure(): Plugin {
  return {
    name: 'assert-web-initial-source-closure',
    apply: 'build',
    enforce: 'post',
    generateBundle(_options, bundle) {
      ...
      const entry = [...chunkManifest.values()].find((chunk) => (
        chunk.isEntry && normalizeModuleId(chunk.facadeModuleId ?? '').endsWith('/clients/web/index.html')
      ))
      if (!entry) throw new Error('Web initial source gate could not find the index.html entry chunk')

      const initialFiles = new Set<string>()
      const visitInitial = (fileName: string) => {
        if (initialFiles.has(fileName)) return
        const chunk = chunkManifest.get(fileName)
        if (!chunk) throw new Error(`Web initial source gate is missing imported chunk ${fileName}`)
      ...
      for (const fileName of initialFiles) {
        ...
        for (const sourceId of Object.keys(chunk.modules)) {
          ...
          if (forbidden) {
            throw new Error(`Web initial source gate found ${forbidden.label} in ${fileName}: ${normalizedId}`)
          }
        }
      }
    },

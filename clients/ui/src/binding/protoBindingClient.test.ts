...
import { ConnectionObservedPath, ConnectionPolicyApplyResultSchema, ConnectionPolicyAvailabilityReason, ConnectionPolicyGetResultSchema, ConnectionPolicyRouteAvailabilitySchema, ConnectionPolicySchema, ConnectionPolicyStateSchema, ConnectionRouteKind, ConnectionSnapshotGetResultSchema, ConnectionSnapshotSchema, EndpointCloudPresenceGetResultSchema, EndpointConnectionEventSchema, EndpointConnectionPhase, EndpointDisconnectResultSchema, EndpointRegistryGetResultSchema, EndpointShareCommitResultSchema, EndpointSharePreviewSchema, EndpointShareReceiveResultSchema, EngineCommandSchema, type EventEnvelope, EventEnvelopeSchema, ExecuteResultSchema, OpenSessionRequestSchema, OpenSessionResultSchema, ResourceStreamClosedEventSchema, ResourceStreamFrameSchema, ResourceStreamFrameType, SessionClosedEventSchema, SessionInvalidateResultSchema, SSHCredentialProvisionResultSchema } from '../generated/bindingpb/client_binding_pb'
...
describe('ProtoBindingSession invalidation', () => {
  it('disconnects an endpoint without requiring a published session handle', async () => {
    const backend = new CancellationBackend()
    let disconnectedEndpoint = ''
    backend.request = async (operation, payload, handle) => {
      if (operation === BindingOperation.ENGINE_COMMAND) {
        ...
        expect(command.command.case).toBe('endpointDisconnect')
        disconnectedEndpoint = command.command.case === 'endpointDisconnect' ? command.command.value.endpointId : ''
        queueMicrotask(() => backend.emit(toBinary(EventEnvelopeSchema, create(EventEnvelopeSchema, {
          event: { case: 'endpointDisconnect', value: create(EndpointDisconnectResultSchema, {
            operationHandle: 1n,
            endpointId: disconnectedEndpoint,
          }) },
        }))))
        return 1n
      }
    ...
    await expect(client.disconnectEndpoint('studio')).resolves.toBeUndefined()
    expect(disconnectedEndpoint).toBe('studio')
    await client.close()
  })
...
describe('ProtoBindingClient engine command boundary', () => {
  it('queries one endpoint Edge presence without opening a session', async () => {
    const backend = new CancellationBackend()
    backend.request = async (operation, payload, handle) => {
      if (operation === BindingOperation.ENGINE_COMMAND) {
        const command = fromBinary(EngineCommandSchema, payload)
        expect(command.command.case).toBe('endpointCloudPresenceGet')
        const endpointId = command.command.case === 'endpointCloudPresenceGet' ? command.command.value.endpointId : ''
        queueMicrotask(() => backend.emit(toBinary(EventEnvelopeSchema, create(EventEnvelopeSchema, {
          event: { case: 'endpointCloudPresenceGet', value: create(EndpointCloudPresenceGetResultSchema, {
            operationHandle: 1n,
            endpointId,
            online: true,
            deviceId: 'device-studio',
            daemonId: 'daemon-studio',
            edgeName: 'CN2',
            edgePublicEndpoint: 'cn2.edge.example:41102',
            locatorSource: 'controller',
            refreshedFromController: true,
          }) },
        }))))
        return 1n
      }
    ...
    await expect(client.getEndpointCloudPresence('studio')).resolves.toMatchObject({
      state: 'online',
      endpointId: 'studio',
      deviceId: 'device-studio',
      daemonId: 'daemon-studio',
...
describe('ProtoBindingConnector route policy', () => {
  ...
  it('uses Go lifecycle events and never presents relay policy as an observed relay path', async () => {
    const states: Array<{ phase: string, relayInUse: boolean, observedPath?: string }> = []
    const client = {
      openSession: vi.fn(async (_request, _signal, onLifecycle) => {
        onLifecycle?.(create(EndpointConnectionEventSchema, {
          endpointId: 'studio', phase: EndpointConnectionPhase.RESOLVING,
        }))
        onLifecycle?.(create(EndpointConnectionEventSchema, {
          endpointId: 'studio', phase: EndpointConnectionPhase.READY,
          observedPath: ConnectionObservedPath.DIRECT, routeSelectionReason: 'only_viable',
        }))
        return { connection: create(ConnectionSnapshotSchema, { observedPath: ConnectionObservedPath.DIRECT }) }
      }),
    } as unknown as ProtoBindingClient
    const connector = new ProtoBindingConnector(() => client, { endpointId: 'studio' })
    ...
    expect(states).toEqual([
      expect.objectContaining({ phase: 'connecting', relayInUse: false }),
      expect.objectContaining({ phase: 'resolving', relayInUse: false }),
      expect.objectContaining({ phase: 'connected', observedPath: 'direct', relayInUse: false }),
      expect.objectContaining({ phase: 'connected', observedPath: 'direct', relayInUse: false }),
    ])
  })
})

describe('ProtoBindingClient endpoint lifecycle', () => {
  it('delivers lifecycle events that arrive before the open operation handle', async () => {
    const backend = new CancellationBackend()
    backend.request = async (operation, _payload, handle) => {
      if (operation === BindingOperation.OPEN_SESSION) {
        backend.emit(toBinary(EventEnvelopeSchema, create(EventEnvelopeSchema, {
          event: { case: 'endpointConnection', value: create(EndpointConnectionEventSchema, {
            operationHandle: 7n, endpointId: 'studio', phase: EndpointConnectionPhase.CONNECTING,
          }) },
        })))
        queueMicrotask(() => backend.emit(toBinary(EventEnvelopeSchema, create(EventEnvelopeSchema, {
          event: { case: 'openSession', value: create(OpenSessionResultSchema, {
            operationHandle: 7n, sessionHandle: 70n,
            session: create(EndpointSessionStampSchema, { endpointId: 'studio', routeId: 'direct', generation: 2n }),
          }) },
        }))))
        return 7n
      }
    ...
    const client = new ProtoBindingClient(backend)
    const phases: EndpointConnectionPhase[] = []

    await client.openSession(
      create(OpenSessionRequestSchema, { endpointId: 'studio' }),
      undefined,
      (event) => phases.push(event.phase),
    )

    expect(phases).toEqual([EndpointConnectionPhase.CONNECTING])
    await client.close()
  })
})

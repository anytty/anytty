...
import { ROUTE_SELECTION_REASONS, type ConnectionPolicy, type ConnectionPolicyState, type RtcConnectionStateSnapshot, type RouteSelectionReason } from '../core/transport'
import type { CloudPresenceSnapshot } from '../connection/cloudPresence'
...
export class ProtoBindingClient {
  ...
  private readonly endpointDisconnectOperations = new Map<bigint, PendingOperation<void>>()
  private readonly endpointCloudPresenceGetOperations = new Map<bigint, PendingOperation<CloudPresenceSnapshot>>()
  ...
  private readonly endpointLifecycleHandlers = new Map<bigint, (event: AnyTTYClientBinding.EndpointConnectionEvent) => void>()
  private readonly earlyEndpointLifecycleEvents = new Map<bigint, AnyTTYClientBinding.EndpointConnectionEvent[]>()
  ...
  async openSession(
    request: AnyTTYClientBinding.OpenSessionRequest,
    signal?: AbortSignal,
    onEndpointLifecycle?: (event: AnyTTYClientBinding.EndpointConnectionEvent) => void,
    ...
    if (onEndpointLifecycle) {
      this.endpointLifecycleHandlers.set(operation, onEndpointLifecycle)
      const early = this.earlyEndpointLifecycleEvents.get(operation)
      if (early) {
        this.earlyEndpointLifecycleEvents.delete(operation)
        for (const event of early) onEndpointLifecycle(event)
      }
    }
    try {
      return await this.waitOperation(this.openOperations, operation, signal)
    } finally {
      this.endpointLifecycleHandlers.delete(operation)
      this.earlyEndpointLifecycleEvents.delete(operation)
    }
  }
  ...
  async disconnectEndpoint(endpointId: string, signal?: AbortSignal): Promise<void> {
    const request = create(AnyTTYClientBinding.EndpointDisconnectRequestSchema, { requestId: crypto.randomUUID(), endpointId })
    const operation = await this.engineCommand(create(AnyTTYClientBinding.EngineCommandSchema, { command: { case: 'endpointDisconnect', value: request } }), signal)
    await this.waitOperation(this.endpointDisconnectOperations, operation, signal)
  }

  async getEndpointCloudPresence(endpointId: string, signal?: AbortSignal): Promise<CloudPresenceSnapshot> {
    const request = create(AnyTTYClientBinding.EndpointCloudPresenceGetRequestSchema, { requestId: crypto.randomUUID(), endpointId })
    const operation = await this.engineCommand(create(AnyTTYClientBinding.EngineCommandSchema, { command: { case: 'endpointCloudPresenceGet', value: request } }), signal)
    return await this.waitOperation(this.endpointCloudPresenceGetOperations, operation, signal)
  }
  ...
  private onEvent(envelope: AnyTTYClientBinding.EventEnvelope): void {
    ...
    switch (event.case) {
      ...
      case 'endpointDisconnect': {
        const pending = this.endpointDisconnectOperations.get(event.value.operationHandle)
        if (!pending) return
        this.endpointDisconnectOperations.delete(event.value.operationHandle)
        if (event.value.error) pending.reject(apiError(event.value.error, 'endpoint disconnect failed'))
        else pending.resolve()
        this.releaseCompletedOperation(event.value.operationHandle)
        return
      }
      case 'endpointCloudPresenceGet': {
        const pending = this.endpointCloudPresenceGetOperations.get(event.value.operationHandle)
        if (!pending) return
        this.endpointCloudPresenceGetOperations.delete(event.value.operationHandle)
        if (event.value.error) pending.reject(apiError(event.value.error, 'endpoint Cloud presence check failed'))
        else pending.resolve(cloudPresenceFromProto(event.value))
        this.releaseCompletedOperation(event.value.operationHandle)
        return
      }
      case 'application':
      ...
      case 'endpointConnection': {
        const handler = this.endpointLifecycleHandlers.get(event.value.operationHandle)
        if (handler) {
          handler(event.value)
          return
        }
        const pending = this.earlyEndpointLifecycleEvents.get(event.value.operationHandle) ?? []
        if (pending.length >= MAX_EARLY_STREAM_EVENTS) {
          this.onClosed(new Error('Proto binding endpoint lifecycle queue overflow'))
          return
        }
        pending.push(event.value)
        this.earlyEndpointLifecycleEvents.set(event.value.operationHandle, pending)
        return
      }
      case 'sessionClosed':
  ...
  private hasPendingOperation(eventCase: AnyTTYClientBinding.EventEnvelope['event']['case'], handle: bigint): boolean {
    switch (eventCase) {
      ...
      case 'endpointDisconnect': return this.endpointDisconnectOperations.has(handle)
      case 'endpointCloudPresenceGet': return this.endpointCloudPresenceGetOperations.has(handle)
      default: return true
    }
  }
  ...
  private rejectAll(error: Error): void {
    for (const registry of [this.openOperations, this.executeOperations, this.importOperations, this.deleteOperations, this.registryGetOperations, this.endpointUpsertOperations, this.endpointDeleteOperations, this.endpointShareReceiveOperations, this.endpointShareCommitOperations, this.sshCredentialProvisionOperations, this.connectionPolicyGetOperations, this.connectionPolicyApplyOperations, this.connectionSnapshotGetOperations, this.sessionInvalidateOperations, this.endpointDisconnectOperations, this.endpointCloudPresenceGetOperations]) {
      for (const pending of registry.values()) pending.reject(error)
      registry.clear()
    }
    this.earlyOperationEvents.clear()
    this.endpointLifecycleHandlers.clear()
    this.earlyEndpointLifecycleEvents.clear()
...
function cloudPresenceFromProto(value: AnyTTYClientBinding.EndpointCloudPresenceGetResult): CloudPresenceSnapshot {
  return {
    state: value.online ? 'online' : 'offline',
    online: value.online,
    endpointId: optionalText(value.endpointId),
    deviceId: optionalText(value.deviceId),
    deviceFingerprint: optionalText(value.deviceFingerprint),
    daemonId: optionalText(value.daemonId),
    edgeId: optionalText(value.edgeId),
    edgeName: optionalText(value.edgeName),
    edgeRegion: optionalText(value.edgeRegion),
    edgePublicEndpoint: optionalText(value.edgePublicEndpoint),
    edgeServerName: optionalText(value.edgeServerName),
    locatorSource: optionalText(value.locatorSource),
    refreshedFromController: value.refreshedFromController,
  }
}
...
export class ProtoBindingConnector {
  ...
  async connect(input: { machineId: string }, options?: { signal?: AbortSignal; forceRelay?: boolean; onStatus?: (status: string) => void; onConnectionState?: (snapshot: RtcConnectionStateSnapshot) => void }): Promise<ProtoClientSession> {
  const endpointId = this.input.endpointId.trim()
    ...
    options?.onConnectionState?.({ machineId: input.machineId, phase: 'connecting', statusText: 'Connecting...', relayInUse: false })
    try {
    ...
    intent: AnyTTYClientBinding.ConnectIntent.INTERACTIVE,
      }), options?.signal, (event) => {
        const snapshot = connectionStateFromEndpointEvent(input.machineId, event)
    ...
    observedPath: observedPathFromProto(session.connection?.observedPath),
    routeSelectionReason: routeSelectionReason(session.connection?.selectionReason),
    relayInUse: session.connection?.observedPath === AnyTTYClientBinding.ConnectionObservedPath.SINGLE_RELAY,
    })
      return session
    } catch (error) {
      options?.onConnectionState?.({ machineId: input.machineId, phase: 'failed', statusText: error instanceof Error ? error.message : 'Connection failed', relayInUse: false })
      throw error
    }
  }
...
function connectionStateFromEndpointEvent(machineId: string, event: AnyTTYClientBinding.EndpointConnectionEvent): RtcConnectionStateSnapshot {
  const phase = rtcPhaseFromEndpointPhase(event.phase)
  const observedPath = observedPathFromProto(event.observedPath)
  return {
    machineId,
    phase,
    observedPath,
    routeSelectionReason: routeSelectionReason(event.routeSelectionReason),
    statusText: endpointPhaseStatus(phase),
    relayInUse: observedPath === 'single_relay',
    error: event.error ? apiError(event.error, 'connection failed') : undefined,
  }
}

function rtcPhaseFromEndpointPhase(phase: AnyTTYClientBinding.EndpointConnectionPhase): RtcConnectionStateSnapshot['phase'] {
  switch (phase) {
    case AnyTTYClientBinding.EndpointConnectionPhase.IDLE: return 'idle'
    case AnyTTYClientBinding.EndpointConnectionPhase.PLANNING: return 'probing'
    case AnyTTYClientBinding.EndpointConnectionPhase.RESOLVING: return 'resolving'
    case AnyTTYClientBinding.EndpointConnectionPhase.SIGNALING: return 'signaling'
    case AnyTTYClientBinding.EndpointConnectionPhase.CONNECTING: return 'connecting'
    case AnyTTYClientBinding.EndpointConnectionPhase.AUTHORIZING: return 'authorizing'
    case AnyTTYClientBinding.EndpointConnectionPhase.READY: return 'connected'
    case AnyTTYClientBinding.EndpointConnectionPhase.OFFLINE: return 'failed'
    default: return 'connecting'
  }
}

function endpointPhaseStatus(phase: RtcConnectionStateSnapshot['phase']): string {
  switch (phase) {
    case 'idle': return 'Idle'
    case 'probing': return 'Planning connection...'
    case 'resolving': return 'Resolving endpoint...'
    case 'signaling': return 'Negotiating connection...'
...
function observedPathFromProto(path: AnyTTYClientBinding.ConnectionObservedPath | undefined): RtcConnectionStateSnapshot['observedPath'] {
  if (path === AnyTTYClientBinding.ConnectionObservedPath.DIRECT) return 'direct'
  if (path === AnyTTYClientBinding.ConnectionObservedPath.SINGLE_RELAY) return 'single_relay'
  return undefined
}

function routeSelectionReason(value: string | undefined): RouteSelectionReason | undefined {
  return ROUTE_SELECTION_REASONS.find((reason) => reason === value)
}

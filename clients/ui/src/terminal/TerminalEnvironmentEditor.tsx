  ...
  return (
    <fieldset className="flex flex-col gap-2">
        ...
        return (
          <div className="grid grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)_3rem] gap-2" key={index}>
            <Input
              ...
              className="min-h-12 min-w-0 bg-zinc-50 text-[15px] text-zinc-900"
            ...
            <Button
              ...
              className="min-h-12 min-w-12 text-zinc-500 active:bg-zinc-100"
              size="icon"
              variant="outline"
      ...
      <div className="grid grid-cols-2 gap-2">
        <Button className="min-h-12 gap-2 px-3 text-[13px] font-semibold" variant="secondary" onClick={() => onChange([...value, '='])}>
          <Plus className="h-4 w-4" />
          {t('workspace.terminalForm.addEnvironment')}
        </Button>
        <Button className="min-h-12 gap-2 px-3 text-[13px] font-semibold" variant="secondary" onClick={() => { setPasteError(null); setPasteOpen(true) }}>
          <ClipboardPaste className="h-4 w-4" />
          {t('workspace.terminalForm.pasteEnvironment')}
        </Button>
      </div>
      {pasteOpen ? (
        <div className="flex flex-col gap-2 rounded-lg border border-[var(--anytty-app-line)] bg-zinc-50 p-3">
          <div className="flex items-center justify-between gap-2">
            <span className="text-[13px] font-semibold text-zinc-700">{t('workspace.terminalForm.pasteEnvironment')}</span>
            <Button aria-label={t('common.close')} className="text-zinc-500 active:bg-zinc-100" size="icon" variant="ghost" onClick={() => setPasteOpen(false)}>
              <X className="h-4 w-4" />
            </Button>
          </div>
          <Textarea
            aria-label={t('workspace.terminalForm.environmentPasteContent')}
            className="min-h-28 resize-y bg-[var(--background)] p-3 font-mono text-[14px] text-zinc-900"
          ...
          <Button className="min-h-12 px-4 text-[14px] font-semibold" onClick={applyPaste}>

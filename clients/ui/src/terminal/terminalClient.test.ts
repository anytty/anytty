...
describe('TerminalClient', () => {
  ...
  it('binds exited terminal history without opening a live channel', async () => {
    const session = createMockTerminalProtocolSession()
    const callbacks = callbacksForTest()
    const client = new TerminalClient(callbacks)

    await client.connectHistory('terminal-exited', session)
    const page = await client.loadScrollback(0, 100, false, { cols: 80 })

    expect(session.openedTerminalIds).toEqual([])
    expect(callbacks.onLifecycle).not.toHaveBeenCalled()
    expect(session.scrollbackRequests).toEqual([{ terminalId: 'terminal-exited', offset: 0, limit: 100, cols: 80 }])
    expect(page.replay).toBe('older')
    client.disconnect()
    expect(session.closedTerminalIds).toEqual([])
  })

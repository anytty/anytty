  ...
  const snapshot = createTerminalInventorySnapshot(
    ...
    rawTerminals.map((terminal) => {
      ...
      const normalized = normalizeTerminal({
        ...
        foreground_process: terminal.foreground_process ?? terminal.foregroundProcess ?? terminal.ForegroundProcess,
        last_output_at: terminal.last_output_at ?? terminal.lastOutputAt ?? terminal.LastOutputAt,
      })

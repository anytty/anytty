...
describe('ConnectionRouteManager', () => {
  ...
  it('edits a single Direct address and saves it for setup and ICE TCP', async () => {
    const user = userEvent.setup()
    const endpoint = testEndpoint()
    const direct = endpoint.routes[0]?.route
    if (direct?.case !== 'directWebrtcTcp') throw new Error('missing Direct fixture')
    direct.value.signalingAddresses = ['192.0.2.10:41120']
    direct.value.iceTcpAddresses = ['192.0.2.10:41120']
    direct.value.serverName = 'legacy-unused.example'
    const save = vi.fn(async (value: EndpointConfigV1) => value)

    render(<ConnectionRouteManager endpointId="studio" adapter={{
      load: vi.fn(async () => endpoint), save, test: vi.fn(async () => {}),
      provisionSSH: vi.fn(async () => { throw new Error('not used') }),
    }} />)

    await screen.findByText('Office LAN')
    await user.click(screen.getAllByRole('button', { name: 'Edit' })[0]!)
    const field = screen.getByLabelText(/Direct address/)
    await user.clear(field)
    await user.type(field, 'direct.example.test:443')
    expect(screen.queryByLabelText('TLS server name')).toBeNull()
    await user.click(screen.getByRole('button', { name: 'Save route' }))

    await waitFor(() => expect(save).toHaveBeenCalledTimes(1))
    const saved = save.mock.calls[0]?.[0].routes.find((route) => route.routeId === 'direct')?.route
    expect(saved?.case).toBe('directWebrtcTcp')
    if (saved?.case !== 'directWebrtcTcp') return
    expect(saved.value.signalingAddresses).toEqual(['direct.example.test:443'])
    expect(saved.value.iceTcpAddresses).toEqual(['direct.example.test:443'])
    expect(saved.value.serverName).toBe('legacy-unused.example')
  })

  it('opens legacy split-port Direct routes in advanced mode', async () => {
    const user = userEvent.setup()
    render(<ConnectionRouteManager endpointId="studio" adapter={{
      load: vi.fn(async () => testEndpoint()), save: vi.fn(async (value) => value), test: vi.fn(async () => {}),
      provisionSSH: vi.fn(async () => { throw new Error('not used') }),
    }} />)

    await screen.findByText('Office LAN')
    await user.click(screen.getAllByRole('button', { name: 'Edit' })[0]!)
    expect(screen.getByLabelText('Setup addresses')).toBeTruthy()
    expect(screen.getByLabelText('ICE TCP addresses')).toBeTruthy()
    expect(screen.getByRole('switch', { name: /Separate setup and ICE ports/ }).getAttribute('data-state')).toBe('checked')
  })

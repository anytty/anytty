...
describe('connectionMessageReducer', () => {
  ...
  it('clears a visible channel error when that channel opens again', () => {
    const failed = reduceConnectionMessage(initialConnectionSnapshot(), {
      type: 'connection.failed',
    ...
    const terminalRecovered = reduceConnectionMessage(failed, {
      type: 'terminal.channelOpen',
      machineId: 'machine-local',
      terminalId: 'term-1',
    })
    expect(terminalRecovered.visibleError).toBeUndefined()

    const fileRecovered = reduceConnectionMessage(failed, {
      type: 'file.channelOpen',
      machineId: 'machine-local',
    })
    expect(fileRecovered.visibleError).toBeUndefined()
  })
})

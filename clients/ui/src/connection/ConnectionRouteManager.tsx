    ...
    <div className="space-y-3 pb-2">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex flex-wrap gap-2">
          {(['direct', 'ssh'] as RouteKind[]).map((kind) => (
            <Button key={kind} className="min-h-12 px-3 text-[12px] font-semibold" variant="secondary" onClick={() => { hapticSelection(); setEditing(null); setAddingKind(kind) }}>
            ...
            <section key={route.routeId} className="py-3">
              <div className="flex min-w-0 items-start gap-3">
                <label className="flex min-h-12 min-w-0 flex-1 items-center gap-3 text-[14px]">
                  <Checkbox checked={route.enabled} disabled={!canDisable || operation.kind === 'saving'} onCheckedChange={() => {
              ...
              <div className="mt-2 flex flex-wrap gap-2 pl-8">
                <Button className="min-h-12 px-3 text-[12px] font-semibold" variant="secondary" onClick={() => { setAddingKind(null); setEditing(create(EndpointRouteConfigV1Schema, route)) }}>{t('workspace.routeManager.edit')}</Button>
                {kind === 'ssh' ? <Button className="min-h-12 px-3 text-[12px] font-semibold" variant="secondary" onClick={() => void provisionSSH(route.routeId)}><KeyRound className="mr-1.5 h-4 w-4" />{t('workspace.routeManager.prepareSSH')}</Button> : null}
                <Button className="min-h-12 px-3 text-[12px] font-semibold text-red-700" variant="secondary" disabled={routes.length <= 1 || operation.kind === 'saving'} onClick={() => {
      ...
      {sshResult?.authorizedKey ? (
        <section className="border-y border-[var(--anytty-app-line)] py-3">
          ...
          <Textarea className="mt-2 min-h-24 resize-y font-mono text-[12px]" readOnly value={sshResult.authorizedKey} aria-label={t('workspace.routeManager.sshPublicKey')} />
          <Button className="mt-2 min-h-12 px-3 text-[12px] font-semibold" variant="secondary" onClick={() => void navigator.clipboard.writeText(sshResult.authorizedKey)}>{t('workspace.routeManager.copyKey')}</Button>
        </section>
      ) : null}
    </div>
  )
...
function RouteEditForm({ endpoint, route, isNew = false, onCancel, onSave }: { endpoint: EndpointConfigV1; route: EndpointRouteConfigV1; isNew?: boolean; onCancel(): void; onSave(route: EndpointRouteConfigV1): void }) {
  ...
  const [advancedDirect, setAdvancedDirect] = useState(() => route.route.case === 'directWebrtcTcp' && !sameAddresses(route.route.value.signalingAddresses, route.route.value.iceTcpAddresses))
  ...
  return (
    <div className="mt-3 space-y-3 border-y border-[var(--anytty-app-line)] bg-zinc-50 px-3 py-3">
      ...
      {kind === 'direct' && direct ? (
        <>
          {advancedDirect ? (
            ...
            <RouteField
              label={t('workspace.routeManager.directAddresses')}
              description={t('workspace.routeManager.directAddressesHint')}
              multiline
              value={direct.signalingAddresses.join('\n')}
              onChange={(value) => {
                const addresses = splitLines(value)
                setRoute({ case: 'directWebrtcTcp', value: create(DirectWebRTCTCPRouteConfigSchema, { ...direct, signalingAddresses: addresses, iceTcpAddresses: addresses }) })
              }}
            />
          )}
          <label className="flex min-h-12 items-center justify-between gap-3 text-[12px] text-zinc-700">
            <span className="min-w-0">
              <span className="block font-semibold">{t('workspace.routeManager.advancedDirect')}</span>
              <span className="mt-0.5 block font-normal leading-4 text-zinc-500">{t('workspace.routeManager.advancedDirectHint')}</span>
            </span>
            <Switch checked={advancedDirect} onCheckedChange={(checked) => {
              setAdvancedDirect(checked)
              if (!checked) {
                const addresses = direct.signalingAddresses.length > 0 ? direct.signalingAddresses : direct.iceTcpAddresses
                setRoute({ case: 'directWebrtcTcp', value: create(DirectWebRTCTCPRouteConfigSchema, { ...direct, signalingAddresses: addresses, iceTcpAddresses: addresses }) })
              }
            }} />
          </label>
      ...
      <div className="flex flex-wrap justify-end gap-2">
        <Button className="min-h-12 px-3 text-[12px] font-semibold" variant="secondary" onClick={onCancel}><X className="mr-1.5 h-4 w-4" />{t('common.cancel')}</Button>
        <Button className="min-h-12 px-3 text-[12px] font-semibold" onClick={() => { hapticImpact(); submit() }}><Save className="mr-1.5 h-4 w-4" />{t('workspace.routeManager.save')}</Button>
      </div>
    </div>
  )
}

function RouteField({ label, description, value, disabled = false, multiline = false, inputMode, onChange }: { label: string; description?: string; value: string; disabled?: boolean; multiline?: boolean; inputMode?: 'numeric'; onChange(value: string): void }) {
  return <label className="block text-[12px] font-semibold text-zinc-700"><span className="mb-1 block">{label}</span>{multiline
    ? <Textarea className="min-h-20 resize-y text-[14px] font-normal" value={value} disabled={disabled} onChange={(event) => onChange(event.target.value)} />
    : <Input className="min-h-12 text-[14px] font-normal" value={value} disabled={disabled} inputMode={inputMode} onChange={(event) => onChange(event.target.value)} />}{description ? <span className="mt-1 block font-normal leading-4 text-zinc-500">{description}</span> : null}</label>
}

function IconAction({ label, disabled = false, onClick, children }: { label: string; disabled?: boolean; onClick(): void; children: ReactNode }) {
  return <Button title={label} aria-label={label} disabled={disabled} size="icon" variant="ghost" className="[&_svg]:h-4 [&_svg]:w-4" onClick={() => { hapticSelection(); onClick() }}>{children}</Button>
}

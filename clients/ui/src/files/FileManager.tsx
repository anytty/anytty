...
import { AlertCircle, ArrowDownAZ, ArrowDownToLine, ArrowUpAZ, ArrowUpFromLine, Bookmark, BookmarkMinus, BookmarkPlus, Box, Check, ChevronRight, ClipboardCopy, ClipboardPaste, Clock, Code2, Eye, EyeOff, File, FileText, FileType, Folder, FolderBookmark, HardDrive, Image, ListChecks, ListFilter, MoreVertical, PlaySquare, RefreshCw, SquarePen, Trash2, X } from 'lucide-react'
  ...
  return (
    ...
    >
      {manager.selectionMode ? (
        ...
        <header className="shrink-0 border-b border-zinc-200/70 bg-white pb-2">
          <div
        ...
        {newDirOpen ? (
          <div className="mx-3 my-2 flex items-center gap-2 rounded-md border border-[var(--anytty-app-line)] bg-zinc-50 p-2">
            ...
            <button
              ...
              className="anytty-app-primary-button !bg-zinc-950 !text-white hover:!bg-zinc-800 disabled:!bg-zinc-200 disabled:!text-zinc-400"
                  ...
                  >
                    {manager.selectionMode ? (
                      <div className="shrink-0 pr-1">
                        <div
                          className={`flex h-6 w-6 items-center justify-center rounded-full border-2 transition-colors ${isSelected ? 'border-zinc-950 bg-zinc-950' : 'border-zinc-300 bg-transparent'}`}
                        >
                    ...
                    >
                      {isRenaming ? (
                        <input
                          ...
                          className="min-h-11 rounded-md border border-zinc-400 bg-white px-2 text-[15px] font-medium text-zinc-900 outline-none focus:border-zinc-950"
                          value={renameName}
        ...
        <div className="absolute inset-0 z-[110] flex items-end bg-black/40 p-3 pb-[calc(env(safe-area-inset-bottom)+0.75rem)] backdrop-blur-sm md:items-center md:justify-center" data-testid="anytty-file-delete-confirm" onClick={() => { hapticSelection(); setDeletePath(null) }}>
          ...
          >
            ...
            <div className="mt-4 grid grid-cols-2 gap-3">
              ...
              <button
                type="button"
                className="h-11 rounded-md border border-red-600 bg-red-600 text-sm font-semibold text-white"
                onClick={() => {
                  hapticImpact()
      ...
      {!manager.selectionMode && clipboardOpen ? (
        <div className="absolute bottom-0 left-0 right-0 z-40 border-t border-zinc-200 bg-white/95 pb-[env(safe-area-inset-bottom)] backdrop-blur-xl" data-testid="anytty-file-clipboard-toolbar">
          <div className="flex h-[52px] items-center gap-2 px-3">
            <button type="button" className="flex h-10 min-w-0 flex-1 items-center justify-center gap-2 rounded-md border border-zinc-200 bg-white px-3 text-sm font-semibold text-zinc-700" onClick={() => { hapticSelection(); manager.setClipboard(null) }}>
              <X className="h-4 w-4" />
              {t('common.cancel')}
            </button>
            <button type="button" className="flex h-10 min-w-0 flex-1 items-center justify-center gap-2 rounded-md bg-zinc-950 px-3 text-sm font-semibold text-white" onClick={() => { hapticImpact(); void manager.paste() }}>
            ...
            <div className="px-5 py-3">
              <label className="flex flex-col gap-2 text-[13px] font-semibold text-zinc-600">
                ...
                <input
                  ...
                  className="h-12 border border-[var(--anytty-app-line)] bg-zinc-50 px-3 text-[16px] font-semibold text-zinc-900 outline-none focus:border-zinc-950 focus:ring-2 focus:ring-zinc-950/15"
                  value={bookmarkAlias}

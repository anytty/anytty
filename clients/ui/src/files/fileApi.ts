...
export interface FileTransferContext {
  ...
  openDownloadedFile?(id: string): void | Promise<void>
...
function protoFileEntry(entry: ProtoFileEntry): FileEntry {
  return {
    ...
    name: entry.name,
    type: entry.type === ProtoFileEntryType.DIRECTORY
      ? (entry.linkTarget ? 'symlink-dir' : 'dir')
      : entry.type === ProtoFileEntryType.SYMLINK ? 'symlink' : entry.type === ProtoFileEntryType.FILE ? 'file' : 'other',

...
export function FileViewerPreview({ preview, streamPreview }: FileViewerPreviewProps) {
  ...
  if (!source) {
    return (
      <div className="flex min-h-56 items-center justify-center gap-3 text-sm font-medium text-zinc-500" role="status">
        <span className="anytty-square-spinner h-5 w-5" aria-hidden="true" />
  ...
  return (
    <div
      className="flex h-full min-h-full w-full flex-col overflow-hidden bg-white"
    ...
    >
      {showToolbar ? <form
        className="flex h-10 w-full shrink-0 items-center justify-center gap-0.5 border-b border-zinc-200 bg-white px-1.5"
      ...
      >
        {showSearch ? <>
          ...
          <input
            ...
            className="h-8 min-w-12 w-full max-w-72 border-0 bg-transparent px-1 text-sm text-zinc-950 outline-none placeholder:text-zinc-400"
          />

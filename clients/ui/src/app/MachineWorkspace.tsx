 ...
 */
export function MachineWorkspace({ api, connector, className, initialMachine, inventoryEvents, connectionStateEvents, subscribeRuntimeInventoryEvents = false, onBack, fileTransfer, terminalSettings: terminalSettingsProp, onNeedsReauthorization, onTerminalSettingsChange, phoneOnline = true, connectionReady = true, connectionRecoveryFailed = false, onRetryConnectionRecovery, singlePane = false }: MachineWorkspaceProps) {
  ...
  const renderTerminalListPage = () => {
    ...
    return (
      <aside
        className={`anytty-app-page relative min-h-0 flex-1 flex-col ${singlePane ? '' : 'md:flex md:w-72 md:flex-none md:border-r md:border-[var(--anytty-app-line)]'} ${page === 'terminal' ? 'hidden' : 'flex'}`}
        data-testid={page === 'terminal' ? undefined : 'anytty-terminal-list-page'}
      >
        <header className={`anytty-app-header flex min-h-12 shrink-0 items-center justify-between border-b px-2 pt-[env(safe-area-inset-top)] ${singlePane ? '' : 'md:pt-0'}`}>
        ...
        {page === 'terminal-list' && showConnectionRecoveryBanner ? (
          ...
          <div className="flex animate-in fade-in slide-in-from-top-1 duration-200 items-center justify-center gap-2 border-b border-zinc-200 bg-zinc-100 px-3 py-1.5">
            <span className="anytty-square-spinner h-3.5 w-3.5 text-zinc-700" aria-hidden="true" />
            <span className="text-[11px] font-medium text-zinc-800">{displayedConnectionStatus || t('workspace.connecting')}</span>
          </div>
        ) : null}
    ...
    >
      ...
      >
        {page === 'terminal-list' ? (
          ...
          <>
        <header
          className={`relative z-30 row-start-1 flex min-h-10 min-w-0 max-w-full shrink-0 items-center justify-between overflow-hidden border-b border-[var(--anytty-border-subtle)] bg-[var(--anytty-surface)] px-0.5 pt-[env(safe-area-inset-top)] ${singlePane ? '' : 'md:hidden'}`}
        ...
        >
          <div className="flex min-w-0 flex-1 items-center gap-1">
            <button
              ...
              className="flex h-10 w-10 shrink-0 items-center justify-center text-[var(--anytty-muted)] transition-colors active:bg-[var(--anytty-surface-raised)]"
            ...
            <button
              ...
              className="flex min-h-10 min-w-0 flex-1 items-center px-1 text-left transition-colors active:bg-[var(--anytty-surface-raised)]"
          ...
          <div className="flex shrink-0 items-center">
            ...
            <button
              ...
              className={`flex h-10 w-10 shrink-0 items-center justify-center transition-colors active:bg-[var(--anytty-surface-raised)] ${splitTerminalId ? 'text-[var(--anytty-text)]' : 'text-[var(--anytty-muted)]'}`}
        ...
        {mobileSheet === 'terminal-menu' ? (
          <div className="absolute inset-0 z-40" onClick={() => { hapticSelection(); setMobileSheet(null) }}>
            <ModalSurface
              ...
              className="absolute right-1.5 top-[calc(env(safe-area-inset-top)+2.5rem)] w-60 origin-top-right overflow-hidden rounded-md border border-zinc-200 bg-white text-zinc-950 shadow-xl animate-in fade-in zoom-in-95 duration-150"
              data-testid="anytty-terminal-menu-sheet"
              onClick={(event) => event.stopPropagation()}
              onRequestClose={() => setMobileSheet(null)}
            >
              <div className="py-1">
                <button type="button" className="flex min-h-11 w-full items-center gap-3 px-3 text-left text-sm font-medium transition-colors hover:bg-zinc-100 active:bg-zinc-200" onClick={() => { hapticImpact(); setMobileSheet(null); setTerminalToolbarModeAndReset('default'); setTerminalToolbarVisibility(true) }}>
                  <SlidersHorizontal className="h-4 w-4 text-zinc-600" />
                  {t('workspace.terminalTools')}
                </button>
                <button type="button" className="flex min-h-11 w-full items-center gap-3 px-3 text-left text-sm font-medium transition-colors hover:bg-zinc-100 active:bg-zinc-200" onClick={() => { hapticSelection(); setMobileSheet(null); void (activeTerminalOwnsResize ? releaseActiveResizeOwner() : acquireActiveResizeOwner()) }}>
                  <Scaling className="h-4 w-4 text-zinc-600" />
                  {t(activeTerminalOwnsResize ? 'workspace.releaseResize' : 'workspace.controlResize')}
                </button>
                <button type="button" className="flex min-h-11 w-full items-center gap-3 border-t border-zinc-200 px-3 text-left text-sm font-medium transition-colors hover:bg-zinc-100 active:bg-zinc-200" onClick={() => { hapticSelection(); setMobileSheet(null); openConnectionInfo() }}>
                  <Info className="h-4 w-4 text-zinc-600" />
                  {t('workspace.connection.label')}
                </button>
                {splitTerminalId ? (
                  <>
                  <button type="button" className="flex min-h-11 w-full items-center gap-3 border-t border-zinc-200 px-3 text-left text-sm font-medium transition-colors hover:bg-zinc-100 active:bg-zinc-200" onClick={() => { hapticImpact(); setSyncSplitInput((current) => !current); setMobileSheet(null) }}>
                    {syncSplitInput ? <Link2 className="h-4 w-4 text-zinc-700" /> : <Link2Off className="h-4 w-4 text-zinc-500" />}
                    {t('workspace.syncInput')}
                  </button>
                  <button type="button" className="flex min-h-11 w-full items-center gap-3 px-3 text-left text-sm font-medium transition-colors hover:bg-zinc-100 active:bg-zinc-200" onClick={() => { hapticImpact(); closeSplitTerminal(); setMobileSheet(null) }}>
                    <PanelBottomClose className="h-4 w-4 text-zinc-600" />
      ...
      {hasOpenedFiles ? (
        <div
          className={`absolute inset-0 z-30 flex flex-col bg-white transition-transform duration-200 ${singlePane ? '' : 'md:left-auto md:right-0 md:w-[450px] md:border-l md:border-[var(--anytty-app-line)]'} ${filesOpen ? (singlePane ? 'translate-y-0 visible' : 'translate-y-0 md:translate-x-0 visible') : (singlePane ? 'translate-y-full invisible' : 'translate-y-full md:translate-y-0 md:translate-x-full invisible')}`}
          data-testid="anytty-machine-files-overlay"
        >
          <div className={`anytty-app-header flex shrink-0 items-center justify-between border-b px-4 pb-2 pt-[calc(env(safe-area-inset-top)+0.5rem)] ${singlePane ? '' : 'md:h-14 md:pb-0 md:pt-0'}`}>

...
final class _BrowserSessionScreenState
  ...
  Future<void> _activateSession(String endpointId, String endpointLabel) {
    final next = _transitionTail.then(
      (_) => _activateSessionNow(endpointId, endpointLabel),
      onError: (_, _) => _activateSessionNow(endpointId, endpointLabel),
    );
  ...
  Future<void> _activateSessionNow(
    ...
    if (hasLiveSession && endpointId == previousEndpointId) return;
    ...
    if (prepared.endpointSubscription != null) {
      _adoptEndpointSession(prepared.endpointSubscription!);
    }
    _proxyLease = lease;
  ...
  Future<bool> _parkLiveSession() async {
    ...
    final parked = await _captureSession(
      sessionId: _activeEndpointId,
      endpointLabel: _activeEndpointLabel,
      controller: controller,
      lease: lease,

...
public final class NativeFilePickerPlugin: CAPPlugin, CAPBridgedPlugin, UIDocumentPickerDelegate {
    ...
    public let pluginMethods: [CAPPluginMethod] = [
        ...
        CAPPluginMethod(name: "readPickedFile", returnType: CAPPluginReturnPromise),
    ...
    @objc func pickFiles(_ call: CAPPluginCall) {
        DispatchQueue.main.async { [weak self] in
            ...
            let mimeType = call.options["mimeType"] as? String
            let contentType = mimeType.flatMap { UTType(mimeType: $0) } ?? .data
    ...
    @objc func readPickedFile(_ call: CAPPluginCall) {
        ioQueue.async {
            do {
                guard let rawURI = call.options["contentUri"] as? String,
                      let url = URL(string: rawURI), url.isFileURL,
                      let maximumBytes = (call.options["maximumBytes"] as? NSNumber)?.intValue,
                      maximumBytes > 0, maximumBytes <= 16 * 1024 * 1024 else { throw FileStoreError.invalidArgument }
                let values = try url.resourceValues(forKeys: [.fileSizeKey, .contentTypeKey, .nameKey])
                if let size = values.fileSize, size > maximumBytes { throw FileStoreError.invalidArgument }
                let data = try Data(contentsOf: url, options: [.mappedIfSafe])
                guard data.count <= maximumBytes else { throw FileStoreError.invalidArgument }
                call.resolve([
            ...
            } catch { call.reject("failed to read picked file", nil, error) }
        }
    }
    ...
    @objc func openFile(_ call: CAPPluginCall) {
        DispatchQueue.main.async { [weak self] in
            guard let self, let viewController = self.bridge?.viewController else {
                call.reject("file opener is unavailable")
                return
            }
            do {
                let url = try self.store.openableURL(uri: self.string(call, "uri"))
                ...
                call.reject("failed to open download", nil, error)
            }
        }
    }
...
private final class IOSDownloadStore {
    ...
    func openableURL(uri: String) throws -> URL {
        guard let candidate = URL(string: uri), candidate.isFileURL else { throw FileStoreError.invalidArgument }
        let root = try downloadDirectory().standardizedFileURL.resolvingSymlinksInPath()
        let file = candidate.standardizedFileURL.resolvingSymlinksInPath()
        guard file.path.hasPrefix(root.path + "/"), manager.fileExists(atPath: file.path) else {
    ...
    private func downloadDestination(name: String) throws -> URL {
        ...
        return try downloadDirectory().appendingPathComponent(normalized, isDirectory: false)
    }

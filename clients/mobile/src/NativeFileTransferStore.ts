...
export class NativeFileTransferStore {
  ...
  async openDownloadedFile(id: string): Promise<void> {
    const transfer = this.transfers.find((item) => item.id === id)
    if (!transfer || transfer.direction !== 'download' || transfer.status !== 'completed' || !transfer.savedUri) {
      throw new Error('completed download is unavailable')
    }
    if (!Capacitor.isNativePlatform()) throw new Error('system file opening requires the native app')

...
// currentAccessStatus 从 pool runtime record 投影 access 角色状态。
// record 缺失时回退为一次 socket 探测；access 独立于 pool 生命周期。
func currentAccessStatus(socketPath string) (accessStatusView, poolRuntimeRecord, error) {
	...
	default:
		return accessStatusView{}, poolRuntimeRecord{}, err
	}
}
...
// accessStartCommand 只启动 access 角色；pool 必须已经在运行。
func accessStartCommand(socket *string) *cobra.Command {
	var jsonOutput bool
	command := &cobra.Command{
		Use: "start", Short: "Start the access process for the running terminal pool", Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
			socketPath := resolveV3Socket(*socket)
			status, _, err := poolStatus(socketPath, "", "")
			if err != nil {
				return err
			}
			if status.State != "running" {
				return &cliError{code: 3, message: "terminal pool is not running; run `anytty pool start` first"}
			}
...
// accessStopCommand 只停止 access 角色；pool 与终端继续运行。
func accessStopCommand(socket *string) *cobra.Command {
	var jsonOutput bool
	command := &cobra.Command{
		Use: "stop", Short: "Stop the access process; terminal pool and terminals keep running", Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
			socketPath := resolveV3Socket(*socket)
			view, record, err := currentAccessStatus(socketPath)
			if err != nil {
			...
			if !jsonOutput {
				fmt.Fprintln(cmd.OutOrStdout(), "Access stopped; terminal pool and terminals keep running")
			}
			return writeAccessStatus(cmd.OutOrStdout(), view, jsonOutput)
		},
	}
...
// pool PID 与终端记录保持不变，客户端重新连接即可。
func accessRestartCommand(socket *string) *cobra.Command {
	var jsonOutput bool
	command := &cobra.Command{
		Use: "restart", Short: "Restart the access process without touching the terminal pool or terminals", Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
			socketPath := resolveV3Socket(*socket)

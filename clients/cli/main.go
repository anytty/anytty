...
// Main executes the anytty CLI with os.Args and returns the process exit
// code. versionOverride is the value injected at link time by the entry
// binary (empty keeps the package default).
func Main(versionOverride string) int {
	if versionOverride != "" {
...
func newRootCmd() *cobra.Command {
	...
	cmd.AddCommand(cloudCommand(&socket, &logFile, &configPath))
	cmd.AddCommand(newWebCommand(&socket, &logFile, &configPath))
	...
	cmd.AddCommand(newUpdateCommand(defaultUpdateCommandRuntime()))

...
func readDaemonRuntimeRecord(path string) (daemonRuntimeRecord, error) {
	...
	if record.AccessPID != 0 && (record.AccessProcessID == "" || record.AccessPID <= 0) {
		return daemonRuntimeRecord{}, fmt.Errorf("daemon runtime record access metadata is invalid")
	}
	return record, nil
}
...
func daemonStatus(socketPath, fallbackLog, fallbackConfig string) (daemonStatusView, daemonRuntimeRecord, error) {
	...
	if dialErr := probeProviderSocket(daemonHealthSocket(socketPath)); dialErr != nil {
		view.State = "starting"
	} else {
		view.State = "running"
	}
...
func newDaemonStartCommand(socket, logFile, configPath *string) *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			// daemon 先起，access 后起；access 失败必须回滚 daemon，避免半栈。
...
// startManagedAccess 启动被 daemon lifecycle 管理的 access 子进程，
...
func startManagedAccess(socketPath string) error {
	...
	identity, err := daemonProcessIdentity(pid)
	if err != nil {
		_ = stopDaemonProcess(pid)
		return err
	}
	if err := patchDaemonRecordAccess(socketPath, pid, identity, logPath); err != nil {
		_ = stopDaemonProcess(pid)
		return err
	}
	deadline := time.Now().Add(5 * time.Second)
	for time.Now().Before(deadline) {
		record, readErr := readDaemonRuntimeRecord(daemonRecordPath(socketPath))
		if readErr == nil && accessProcessState(record) == "running" {
			return nil
		}
		time.Sleep(25 * time.Millisecond)
	}
	record, _ := readDaemonRuntimeRecord(daemonRecordPath(socketPath))

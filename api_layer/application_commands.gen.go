...
func (service *Service) dispatchPlatformCommand(ctx context.Context, requestID string, session *apipb.EndpointSessionStamp, command *apipb.CommandEnvelope) *apipb.ResultEnvelope {
	...
	case *apipb.CommandEnvelope_RemoteCloudStatus:
		result, err := service.platform.RemoteCloudStatus(ctx, session, value.RemoteCloudStatus)
		if err != nil || result == nil {
			return errorResult(requestID, session, apimapping.ErrorToProto(err, true))
		}
	...
	case *apipb.CommandEnvelope_RemoteCloudEnable:
		result, err := service.platform.RemoteCloudEnable(ctx, session, value.RemoteCloudEnable)
		if err != nil || result == nil {
			return errorResult(requestID, session, apimapping.ErrorToProto(err, true))
		}
		return &apipb.ResultEnvelope{RequestId: requestID, OriginSession: cloneSession(session), Result: &apipb.ResultEnvelope_RemoteCloudStatus{RemoteCloudStatus: result}}
	case *apipb.CommandEnvelope_RemoteCloudDisable:
		result, err := service.platform.RemoteCloudDisable(ctx, session, value.RemoteCloudDisable)
		if err != nil || result == nil {
			return errorResult(requestID, session, apimapping.ErrorToProto(err, true))
		}
		return &apipb.ResultEnvelope{RequestId: requestID, OriginSession: cloneSession(session), Result: &apipb.ResultEnvelope_RemoteCloudStatus{RemoteCloudStatus: result}}

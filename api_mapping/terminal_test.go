...
func TestValidateTerminalAttachRejectsObserverOwnerPolicy(t *testing.T) {
	contextMessage := terminalRequestContext("request-observer-owner")
	command := &apipb.CommandEnvelope{Context: contextMessage, Command: &apipb.CommandEnvelope_TerminalAttach{TerminalAttach: &apipb.TerminalAttachCommand{
		Terminal: &apipb.TerminalRef{EndpointId: "studio", TerminalId: "term-1"}, Mode: apipb.AttachmentMode_ATTACHMENT_MODE_OBSERVER,
		ResizePolicy: apipb.ResizePolicy_RESIZE_POLICY_OWNER, SurfaceId: "surface", ViewId: "view",
		Operation: &apipb.OperationStamp{Session: contextMessage.GetSession(), OperationId: "attach-observer"},
	}}}
	if err := ValidateTerminalCommand(command); err == nil {
		t.Fatal("observer mode with owner policy must fail")
	}
}

...
func ValidateAccessRemoteCommand(command *apipb.CommandEnvelope) error {
	...
	case *apipb.CommandEnvelope_ClientAccessList,
		...
		*apipb.CommandEnvelope_RemoteLocalDisable, *apipb.CommandEnvelope_RemoteCloudStatus,
		*apipb.CommandEnvelope_RemoteCloudEnable, *apipb.CommandEnvelope_RemoteCloudDisable,
...
func ClientAccessTicketRequestFromProto(command *apipb.ClientAccessTicketCreateCommand) accesscontract.ClientAccessTicketRequest {
	...
	return accesscontract.ClientAccessTicketRequest{Label: request.GetLabel(), AccessLabel: request.GetAccessLabel(), Scope: clientAccessScopeFromProto(request.GetScope()), TicketTTL: time.Duration(request.GetTicketTtlSeconds()) * time.Second, GrantLifetime: time.Duration(request.GetGrantLifetimeSeconds()) * time.Second, Routes: routes}
}

// ClientAccessIdentityToProto 转为公开 DeviceIdentity 投影，私钥不在输入模型中。
func ClientAccessIdentityToProto(identity accesscontract.ClientAccessIdentity) *remoteauthpb.ClientAccessIdentityResult {
...
func ClientAccessTicketToProto(ticket accesscontract.ClientAccessTicket) *remoteauthpb.ClientAccessTicketCreateResult {
...
func ClientAccessRecordToProto(record accesscontract.ClientAccessRecord) *remoteauthpb.ClientAccessRecord {
...
func ClientAccessIdentityResultToProto(identity accesscontract.ClientAccessIdentity) *apipb.ClientAccessIdentityResult {
...
func ClientAccessListToProto(records []accesscontract.ClientAccessRecord) *apipb.ClientAccessListResult {
...
func ClientAccessTicketResultToProto(ticket accesscontract.ClientAccessTicket) *apipb.ClientAccessTicketCreateResult {
...
func ClientAccessRevokeToProto(record accesscontract.ClientAccessRecord) *apipb.ClientAccessRevokeResult {
...
func RemotePairStartRequestFromProto(command *apipb.RemotePairStartCommand) accesscontract.RemotePairStartRequest {
	return accesscontract.RemotePairStartRequest{LocalPairURL: command.GetLocalPairUrl(), TTLSeconds: int(command.GetTtlSeconds()), AuthorizationTTLSeconds: int(command.GetAuthTtlSeconds())}
}

// RemoteLocalEnableRequestFromProto 转为 core-native local remote runtime request。
func RemoteLocalEnableRequestFromProto(command *apipb.RemoteLocalEnableCommand) accesscontract.RemoteLocalEnableRequest {
	return accesscontract.RemoteLocalEnableRequest{LocalWebAddress: command.GetLocalWebAddress(), ICETCPAddress: command.GetIceTcpAddress(), HubURLs: append([]string(nil), command.GetHubUrls()...), ControlURL: command.GetControlUrl(), AccessToken: command.GetAccessToken(), Region: command.GetRegion(), LocalWebPassword: append([]byte(nil), command.GetLocalWebPassword()...)}
}

// RemoteStatusToProto 转为公共 remote runtime 状态。
func RemoteStatusToProto(status accesscontract.RemoteStatus) *apipb.RemoteStatusResult {
...
func RemotePairStartToProto(result accesscontract.RemotePairStartResult) *apipb.RemotePairStartResult {
...
func RemoteLocalStatusToProto(status accesscontract.RemoteLocalStatus) *apipb.RemoteLocalStatusResult {
...
func RemoteCloudStatusToProto(status accesscontract.RemoteCloudStatus) *apipb.RemoteCloudStatusResult {
	return &apipb.RemoteCloudStatusResult{
		Enrolled: status.Enrolled, Enabled: status.Enabled, Running: status.Running, State: status.State, Detail: status.Detail,
		DaemonId: status.DaemonID, AccountId: status.AccountID,
		...
		LifecycleState: status.LifecycleState, LifecycleRevision: status.LifecycleRevision,
		Ready: status.Ready, ActiveSessions: int32(status.ActiveSessions),
		EnrolledAtUnixNano: unixNanoOrZero(status.EnrolledAt), UpdatedAtUnixNano: unixNanoOrZero(status.UpdatedAt),
		RecordPath: status.RecordPath, DisabledPath: status.DisabledPath,
	}
}

func clientAccessScopeFromProto(scope *remoteauthpb.ClientAccessScope) accesscontract.ClientAccessScope {
	if scope == nil {
		return accesscontract.ClientAccessScope{}
	}
	return accesscontract.ClientAccessScope{AllowDaemon: scope.GetAllowDaemon(), TerminalID: scope.GetTerminalId(), MachineEventsOnly: scope.GetMachineEventsOnly(), FileReadMetadata: scope.GetFileReadMetadata(), FileReadContent: scope.GetFileReadContent(), FileWriteContent: scope.GetFileWriteContent(), FileMutate: scope.GetFileMutate(), ManageClientAccess: scope.GetManageClientAccess()}
}

func clientAccessScopeToProto(scope accesscontract.ClientAccessScope) *remoteauthpb.ClientAccessScope {

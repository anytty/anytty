...
func (channel *Channel) Send(payload []byte) error {
	// The shared transport records a concrete send failure before closing this channel.
	// Publishing an untyped close here would race that error into a generic EOF.
...
func (channel *Channel) Close() error {
	...
	// Pion may not deliver OnClose after a local close when the remote peer is a
	// network black hole. Publish the local lifecycle boundary immediately so
	// protocol waiters and SessionOwner cannot remain connected indefinitely.
	channel.notifyClosed()
	return err
}

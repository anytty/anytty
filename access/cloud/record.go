// Package cloud 实现 access 进程侧唯一 AnyTTY Cloud owner，服务同一终端 pool。
package cloud
...
// EnrollmentRecord 是 pool 唯一允许持久化的 Cloud 状态。
...
// LoadRecord 严格加载 owner-only JSON；文件不存在表示 pool 尚未启用 Cloud。

...
func requiresPoolRelayCandidate(offer *cloudv1.AgentOffer) bool {
...
// filterPoolRelayICEURLs keeps both peers on the transport selected by the
...
func filterPoolRelayICEURLs(values []string, preference cloudv1.RelayTransport) ([]string, error) {
	switch preference {
...
func hasPoolTURNICEURL(values []string) bool {
	for _, value := range values {

...
func TestRequiresDaemonRelayCandidate(t *testing.T) {
	for _, transport := range []cloudv1.RelayTransport{
		...
		for _, hasRelay := range []bool{false, true} {
			...
			if got := requiresPoolRelayCandidate(offer); got != want {
				t.Fatalf("transport=%s relay=%t: got %t, want %t", transport, hasRelay, got, want)
			}
		}
	}
	if requiresPoolRelayCandidate(nil) {
...
func TestFilterDaemonRelayICEURLs(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			got, err := filterPoolRelayICEURLs(values, test.preference)
			if err != nil {
				t.Fatalf("filterPoolRelayICEURLs() error = %v", err)
			}
			if !reflect.DeepEqual(got, test.want) {
				t.Fatalf("filterPoolRelayICEURLs() = %#v, want %#v", got, test.want)
			}
		})
	}
}
...
func TestFilterDaemonRelayICEURLsRejectsInvalidOrMissingTransport(t *testing.T) {
	if _, err := filterPoolRelayICEURLs([]string{"turn:relay.example:3478"}, cloudv1.RelayTransport(99)); err == nil {
		t.Fatal("invalid Relay transport was accepted")
	}
	if urls, err := filterPoolRelayICEURLs([]string{"stun:relay.example:3478", "turn:relay.example:3478?transport=tcp"}, cloudv1.RelayTransport_RELAY_TRANSPORT_UDP); err != nil {
		t.Fatalf("filterPoolRelayICEURLs() error = %v", err)
	} else if hasPoolTURNICEURL(urls) {

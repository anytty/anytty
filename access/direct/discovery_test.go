...
func TestStartDirectDiscoveryPublishesWildcardListenerIdentity(t *testing.T) {
	originalRegister, originalAddresses := registerDirectDiscovery, PrivateLANAddresses
	defer func() { registerDirectDiscovery, PrivateLANAddresses = originalRegister, originalAddresses }()
	PrivateLANAddresses = func() ([]string, error) { return []string{"192.168.1.9", "192.168.1.8"}, nil }
...
func TestStartDirectDiscoveryRegistersWhenLANAppearsAfterStartup(t *testing.T) {
	originalRegister, originalAddresses, originalPeriod := registerDirectDiscovery, PrivateLANAddresses, directDiscoveryRefreshPeriod
	defer func() {
		registerDirectDiscovery, PrivateLANAddresses, directDiscoveryRefreshPeriod = originalRegister, originalAddresses, originalPeriod
	}()
	directDiscoveryRefreshPeriod = time.Millisecond
	var lookups atomic.Int32
	PrivateLANAddresses = func() ([]string, error) {
		if lookups.Add(1) == 1 {
			return nil, nil
		}

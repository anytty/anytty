...
> 范围：`access/`（协议服务器 + 能力路由 + runtime）与 `pool/`（terminal provider）。
> 状态：Phase 0–4 完成（2026-09-19）。access 是唯一客户端入口与能力 owner；
> pool 是纯终端 provider。access wire 保持不变。
...
pool，`<canonical>.provider`）。pool 不认识远程会话、不持有身份/账本/文件，
也不再有网络出口。

## 1. 职责边界

### 1.1 terminal pool（`pool/{core,provider}`；access 侧边界是 `access/{remote,cloud}`）

- 只监听本地 unix socket `<canonical>.provider`（`0600`）；
  无网络 listener（`TestPoolHasNoTCPListeners`）。
...
- `client_access.*` / `cloud.*` / `file.*` / `storage.*` / browser proxy 命令
  fail closed（unsupported）；pool 不再挂载 ClientAccessService/RemoteService，
  也没有 control RPC 客户端。
- 旧的 file/storage/browser 实现与测试已删除；生成的 API mapping 因仓库缺失
  `scripts/generate_application_api.go` 未重生成，pool port 上保留 fail-closed
  stub（见 HANDOFF §7）。
...
| `access/provider/pool` | pool provider：owner-only dial + Hello v7，命令 correlation 原样透传 |
...
```
 客户端 ── access wire ──▶ access/server（Hello/请求/流/事件）
                              │ FamilyTerminal ──▶ provider ──▶ pool（PTY/history/live）
...
- relay 路径字节透明：access 不解析 frame；授权是网络层边界。relay 的本地
  ...
  `.provider`；pool 只在 provider 协议上服务 access。

## 3. 能力路由（`access/server/router.go`）

| family | 命令 | 处理 |
|---|---|---|
| FamilyTerminal | terminal.*、history.*、live、events、path.* | provider（pool） |
...
- 远程：remoteauth（grant/DeviceHello/DTLS binding）在 access 完成；pool
  只看到 access 的内部连接。
...
```bash
# 推荐：同一 CLI 管理两个进程（pool 先起，access 后起；日志分开）
anytty --socket /tmp/anytty.sock pool start
anytty --socket /tmp/anytty.sock pool status --json
anytty --socket /tmp/anytty.sock pool stop

# 前台/独立进程
anytty --socket /tmp/anytty.sock pool run            # 绑定 /tmp/anytty.sock.provider
...
日志只写文件：pool 日志 `anytty.log`；access 日志

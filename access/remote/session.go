// Package remote owns the authorized remote DataChannel-to-core-v2 boundary.
package remote
...
// TransportServer 是 remote-v2 允许调用的 core-v2 transport 边界。
// access owner 完成 remoteauth 后才调用它；pool 侧连接与本地客户端等价（plan A 无 scope）。
type TransportServer interface {
	// ServeTransport 在同一条 DataChannel 上服务 anytty protocol。
...
// Identity、grant expiry/revoke 和 scope mapping 都属于 pool；信令结果不能预授权 session。
type SessionAcceptor struct {
	Core        TransportServer
...
// peerDTLSFingerprint 必须来自当前 Pion 本端 DTLSTransport；失败会关闭当前 channel，且不创建 protocol session 或 fallback。
func (acceptor SessionAcceptor) ServeDataChannel(ctx context.Context, connection transport.Transport, peerDTLSFingerprint string) error {
	binding, err := remoteauth.DTLSChannelBinding(peerDTLSFingerprint)
	if err != nil {
		if connection != nil {
			_ = connection.Close()
		}
...
func (acceptor SessionAcceptor) ServeBoundTransport(ctx context.Context, connection transport.Transport, binding remoteauth.ChannelBinding) error {
	if acceptor.Core == nil {
		return fmt.Errorf("remote pool core transport server is not configured")
	}
	if connection == nil {
		return fmt.Errorf("remote pool data channel transport is not configured")
	}
	handedToCore := false
	defer func() {
		if !handedToCore {
	...
	if acceptor.AccessStore == nil || !acceptor.AccessStore.Available() {
		return fmt.Errorf("remote pool client access store is not configured")
	}
	if binding.Kind == remoteauthpb.ChannelBindingKind_CHANNEL_BINDING_KIND_LOCAL_UNIX {
		return fmt.Errorf("remote pool session ingress rejects local Unix binding")
	}
	...
	handedToCore = true
	ctx = sessions.WithRemoteSessionInfo(ctx, sessions.RemoteSessionInfo{
		GrantID: claims.GrantID, PrincipalID: claims.SubjectKeyFingerprint, ExpiresAt: claims.ExpiresAt,
	})
	return acceptor.Core.ServeTransport(ctx, connection)
}

// GrantActive 让 Direct signaling 在分配 PeerConnection 前复用 pool-local 持久 grant 真值。
...
// PairingAcceptor 是 owner-only local Unix pairing socket 的受限 pool 入口。
...
func (acceptor PairingAcceptor) ServeBoundTransport(ctx context.Context, connection transport.Transport, binding remoteauth.ChannelBinding) error {
	if connection == nil {
		return fmt.Errorf("remote pool pairing transport is not configured")
	}
	if acceptor.AccessStore == nil || !acceptor.AccessStore.Available() {
		return fmt.Errorf("remote pool pairing store is not configured")
	}
	if binding.Kind != remoteauthpb.ChannelBindingKind_CHANNEL_BINDING_KIND_LOCAL_UNIX {

...
func TestSessionAcceptorAuthenticatesBeforeServingTransport(t *testing.T) {
	...
	if core.calls != 1 {
		t.Fatalf("unexpected core calls: %d", core.calls)
	}
...
func (core *recordingCore) ServeTransport(_ context.Context, conn transport.Transport) error {
	core.calls++
	return conn.Close()
}

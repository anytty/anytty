...
func TestSemanticTapInputRecordsRemainBoundedAndOrdered(t *testing.T) {
	tap := NewLineHistorySemanticTap("term-bounded-input-records", Size{Cols: 80, Rows: 24}, nil)
	total := semanticTapInputRecordCapacity*4 + 17
	for index := 0; index < total; index++ {
		if _, err := tap.ApplyPTYWrite([]byte("x")); err != nil {
			t.Fatalf("apply PTY write %d: %v", index, err)
		}
	}

	records := tap.InputRecords()
	if len(records) != semanticTapInputRecordCapacity {
		t.Fatalf("input records=%d, want fixed capacity %d", len(records), semanticTapInputRecordCapacity)
	}
	wantFirst := uint64(total - semanticTapInputRecordCapacity + 1)
	for index, record := range records {
		wantSeq := wantFirst + uint64(index)
		if record.Seq != wantSeq || record.Kind != SemanticTapInputWrite {
			t.Fatalf("input record %d=%#v, want seq=%d kind=%s", index, record, wantSeq, SemanticTapInputWrite)
		}
	}
}

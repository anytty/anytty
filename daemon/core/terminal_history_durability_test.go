...
func TestHistoryReadsSyncOnlyWhenCreatingFrozenSnapshot(t *testing.T) {
	...
	if _, err := server.TerminalHistoryCopy(context.Background(), terminalID, history.HistoryCopyRequest{TerminalID: terminalID}); !errors.Is(err, history.ErrHistoryInvalidMutation) {
		t.Fatalf("tokenless copy error = %v, want %v", err, history.ErrHistoryInvalidMutation)
	}
	_, syncCalls, _ := storage.counts()
	if syncCalls != 0 {
	...
	if _, err := server.TerminalHistoryWindow(context.Background(), terminalID, history.HistoryWindowRequest{
		TerminalID: terminalID,
	...
	if _, err := server.TerminalHistoryCopy(context.Background(), terminalID, history.HistoryCopyRequest{
		TerminalID: terminalID,
		Token:      snapshot.Token,
	}); err != nil {
		t.Fatal(err)

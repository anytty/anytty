...
- 移动 App 不登录、不关联 Cloud 账号。扫码建立身份 pin 后，可通过 DNS-SD 查找该已配对设备的短期局域网地址；发现结果不创建新设备，也不替代配对。
...
Direct 默认只占一个 TCP 端口，协商帧与 ICE-TCP/STUN 帧由 daemon 在首帧处分流。外部映射可只发布一个地址：

```sh
anytty pair create --route direct --direct-address direct.example.com:443 --qr-file ./anytty-pair.png
```

为兼容旧部署，`--signaling-address` 与 `--ice-tcp-address` 仍可成对指定不同端口。Direct 不使用 `server-name`；Cloud Controller/Edge 的 TLS server name 是独立概念。

...
> 能力 owner；pool = 纯 terminal provider，只在 `.provider` 上说 provider
> 协议；CLI 默认入口 = `clients/tui` 的 tui2 宿主 + tui2-shell；旧 pool
...
- **pool 是纯终端 provider**：只监听内部 owner-only socket，无身份、无账本、
  无文件、无 storage、无网络出口；`client_access.*` / `cloud.*` / `file.*` /
  `storage.*` / browser proxy 在 pool 侧 fail closed。
...
| `access/provider/pool` | provider 适配器：apipb 命令 → providerv1 typed 调用，附件流 bootstrap、terminal event 投影、provider 错误码映射 |
...
### 1.2 terminal pool（`pool/{core,provider}`；access 侧边界是 `access/{remote,cloud}`）

- 只监听 `<canonical>.provider`（`0600`，本地 owner 等价）；
  `TestPoolHasNoTCPListeners` 仍成立。
...
| `<canonical>.control` | — | 已删除（不再有 pool→access 反向 RPC） |

## 2. 生命周期与 CLI

- `anytty pool start`：先起 pool（provider 协议，绑定 `.provider`），
  就绪探测走 provider Hello，再起 access（canonical）；写入同一 runtime record 的
  pool + access PID；失败回滚，避免半栈。
- `anytty pool stop`：先停 access，再停 pool；status 同时报告两者。
- 日志分开：`anytty.log`（pool）与 `anytty-access.log`（access）。
- 兼容：隐藏别名 `anytty daemon ...` 仍可用，但会打印一行弃用提示并转发到 `anytty pool ...`；env 先读 `ANYTTY_POOL_*` 再回退 `ANYTTY_DAEMON_*`，配置先读 `pool:` 段再回退 `daemon:` 段，runtime record 缺失时回退读取旧 `<sock>.daemon.json`。
...
- CLI 客户端 auto-start 会成对拉起 pool+access，并等待 provider 就绪后才暴露
  canonical 入口。
...
```
anytty/
├─ access/{accessrun,server,files,proxy,storage,provider/{terminal,pool},
│          runtime,direct,gateway,localstate,sessions,engine,...,cmd/anytty-access}
├─ pool/core + access/{remote,cloud}
...
单二进制 / 角色隔离：`anytty` 同一二进制以两个独立进程运行 pool（`.provider`）与 access
（canonical socket）。`anytty access start|stop|restart` 只动 access；`anytty pool restart
--keep-access` 只动 pool；access 崩溃不影响终端。详见 `access/docs/ARCHITECTURE.zh-CN.md` §9。

```bash
go build ./... && go vet ./...
ANYTTY_ALLOW_NESTED=1 go test -count=1 -p 4 ./...
go test -count=1 -race -p 2 ./access/... ./pool/... ./clients/cli/...
...
# 开发：同一二进制管理两进程
go run ./cmd/anytty --socket /tmp/anytty.sock pool start
...
- access wire 只允许字节透明或**可选字段**扩展；Hello v7/pairing/Direct/Cloud
  握手不改。
- pool 永不暴露公网、不持有 identity/store/files/storage；access 是唯一网络
  入口与授权 owner。
...
- 删除 `proto/access/apipb/plugin.proto` + `plugin.pb.go`、`application.proto` 的
  ...
  `api_mapping` 的 plugin capability 分支和 pool 的
...
**T1 Slice A1（provider 协议 + pool provider server）**
...
- 新增 `pool/provider`：provider server（Hello/请求预算/typed 错误映射）+
  同步 Client；只调用 `core.Server`/`Terminal` 的终端服务，新增最小 core hook
  `Server.TerminalDefaultsSnapshot`/`Server.ListPathDirectories`。
- additive：旧 pool client 协议与 access 路径完全未动；provider server 仅在
  测试中监听独立 socket。
- **Slice A2（本轮完成）**：provider proto 增加 typed attach/detach/input/
  ...
  `pool/provider` 增加 server 级 attachment 仲裁 registry、session 级 channel/
...
- **Slice A3（本轮完成）**：provider proto 增加 provider-native history
  ...
  event subscription（subscribe/release + terminal lifecycle events）；pool
...
- **Slice B（本轮完成）**：access 全量切换到 provider 协议。
  `access/provider/pool` 从旧的 v7 client engine 改写为 providerv1 适配器
  ...
  不向 provider 订阅）。`clients/cli` pool 改为 `core.Server.Start(ctx)` +
  `pool/provider.Server` 监听 `.provider`；pool start/status 健康探针改为
  provider Hello；accessrun 的 relay 目标从 `.provider` 改指 canonical access
  （remote 客户端仍说 access wire）。新增 `access/provider/pool` 适配器测试
  （命令分发/错误映射/事件投影/storage-only release），access e2e、clients/cli
  smokes 全绿。
- **Slice C（本轮完成）**：删除旧 pool client-protocol 栈。`pool/core` 删除
  ...
  去掉对应生成目标；`testkit` 旧测试 helper（原 daemon.go）删除。
- access 归属类型下沉：新增 `access/contract`（ClientAccess*/Remote* DTO 与
  ...
  `access/server` 不再 import `pool/core`/`access/remote`，`api_mapping`、
  `access/runtime`、`clients/cli` 改用 access-owned 类型，行为不变。
- 覆盖迁移：旧协议测试删除约 2.8k 行；语义仍存在的部分迁到 provider：
  `pool/provider/ownership_test.go`（owner transfer/epoch fence、promotion、
...
- **T4（本轮完成）**：CLI 默认入口切到新 TUI。`anytty`（根命令）与 `anytty
  ...
  pool+access；显式 `--socket` 通过临时 registry override 注入；`--config`
  ...
  （get/set/unset/show/validate/paths）。pool/history 的 `--config` 仍是
  `tui-v3.yaml` 的 `pool:` 段（换用新解析器，格式/环境变量/校验不变）。
...
- 用户可见差异：`--config` 对 TUI 入口现在是 tui2 JSON（旧 `tui-v3.yaml`
  ...
  安装 `tui-v3.yaml`（现为 pool 配置模板）。
- **T5（本轮完成，最终清理）**：`deadcode`（经镜像 `golang.org/x/tools/cmd/deadcode@latest`）
  对 `./cmd/anytty`（默认/dev tag）、`anytty pool run` 入口、`./access/cmd/anytty-access`、
...
1. **protocolserver 抽包不再需要**：pool 客户端协议栈已删除，access 使用

...
- Windows 可执行文件现在内嵌无损 ConPTY 运行时，可保留任意 CLI 输出中的带样式空白单元格，以及完整终端历史所需的原始 VT 滚动数据流。
- 远程应用请求现在会在嵌套运行时适配器之间保留唯一的关联标识，避免并发请求、连接复用或资源清理后 TUI 偶发失败。

## [0.0.1-beta.10] - 2026-09-04

### 新增

- 新增远程 Web 会话浏览器界面，支持按代理隔离、持久化会话状态、多标签快照、历史记录建议、阅读模式、电脑模式和离线恢复提示。
- endpoint 与 terminal 页面新增浏览器入口，并统一浏览器实例及 App/系统返回行为。

### 变更

- 将浏览器导航整合为一层紧凑工具栏，移动端支持标签页切换，并接入现有 terminal 花瓣菜单。
- Android debug 与 release 签名配置同时启用 V1 和 V2 APK 签名方案，提升部分 OEM 安装器的兼容性。

### 修复

- terminal 链接现在默认使用 AnyTTY 浏览器打开，同时保留 `mailto:` 与 `tel:` 的系统外部打开行为。
- 从 terminal 进入浏览器时不再创建第二个 workspace；退出浏览器后保留内存中的 WebView 状态。

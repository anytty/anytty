...
func newActiveTCPConn(
	...
	go func() {
		...
		var err error
		if len(networks) > 0 {
			if scoped, ok := networks[0].(interface {
				DialTCPContext(context.Context, string, *net.TCPAddr, *net.TCPAddr) (net.Conn, error)
			}); ok {
				conn, err = scoped.DialTCPContext(ctx, "tcp", laddr, net.TCPAddrFromAddrPort(remoteAddress))
			} else {

...
func (a *Agent) gatherCandidatesSrflx(ctx context.Context, urls []*stun.URI, networkTypes []NetworkType) {
	a.gatherOnNetworks(ctx, func(network transport.Net) {
		a.gatherCandidatesSrflxOnNetwork(ctx, urls, networkTypes, network)
	})
}

// Optional platform extension: each transport binds DNS and sockets to one
// network. Host candidate pairs still share the same upstream ICE agent.
func (a *Agent) gatherOnNetworks(ctx context.Context, gather func(transport.Net)) {
	provider, ok := a.net.(interface{ ICEGatherNetworks() []transport.Net })
	if !ok {
		gather(a.net)
		return
	}
	var wg sync.WaitGroup
	for _, network := range provider.ICEGatherNetworks() {
		wg.Add(1)
		go func(network transport.Net) {
			defer wg.Done()
			select {
			case networkGatherBudget <- struct{}{}:
				defer func() { <-networkGatherBudget }()
				gather(network)
			case <-ctx.Done():
			}
		}(network)
...
var networkGatherBudget = make(chan struct{}, 12)

func (a *Agent) gatherCandidatesSrflxOnNetwork(ctx context.Context, urls []*stun.URI, networkTypes []NetworkType, networkTransport transport.Net) {
	var wg sync.WaitGroup
	defer wg.Wait()
	...
	for _, networkType := range networkTypes {
		...
		for i := range urls {
			...
			go func(url stun.URI, network string) {
				...
				hostPort := fmt.Sprintf("%s:%d", url.Host, url.Port)
				serverAddr, err := networkTransport.ResolveUDPAddr(network, hostPort)
				if err != nil {
					a.log.Debugf("Failed to resolve STUN host: %s %s: %v", network, hostPort, err)
...
func (a *Agent) gatherCandidatesRelay(ctx context.Context, urls []*stun.URI) {
	a.gatherOnNetworks(ctx, func(network transport.Net) {
		a.gatherCandidatesRelayOnNetwork(ctx, urls, network)
	})
}

func (a *Agent) gatherCandidatesRelayOnNetwork(ctx context.Context, urls []*stun.URI, networkTransport transport.Net) {
	var wg sync.WaitGroup
	defer wg.Wait()

	network := NetworkTypeUDP4.String()
	_, ifaces, _ := localInterfaces(networkTransport, a.interfaceFilter, a.ipFilter, a.networkTypes, a.includeLoopback)
	...
	for _, url := range urls {
		...
		go func(url stun.URI) {
			...
			case url.Proto == stun.ProtoTypeUDP && url.Scheme == stun.SchemeTypeTURN:
				if locConn, err = networkTransport.ListenPacket(network, "0.0.0.0:0"); err != nil {
					a.log.Warnf("Failed to listen %s: %v", network, err)
			...
			case url.Proto == stun.ProtoTypeTCP && url.Scheme == stun.SchemeTypeTURN:
				tcpAddr, connectErr := networkTransport.ResolveTCPAddr(NetworkTypeTCP4.String(), turnServerAddr)
				if connectErr != nil {
				...
				conn, connectErr := networkTransport.DialTCP(NetworkTypeTCP4.String(), nil, tcpAddr)
				if connectErr != nil {
			...
			case url.Proto == stun.ProtoTypeUDP && url.Scheme == stun.SchemeTypeTURNS:
				udpAddr, connectErr := networkTransport.ResolveUDPAddr(network, turnServerAddr)
				if connectErr != nil {
				...
				udpConn, dialErr := networkTransport.DialUDP("udp", nil, udpAddr)
				if dialErr != nil {
			...
			case url.Proto == stun.ProtoTypeTCP && url.Scheme == stun.SchemeTypeTURNS:
				tcpAddr, resolvErr := networkTransport.ResolveTCPAddr(NetworkTypeTCP4.String(), turnServerAddr)
				if resolvErr != nil {
				...
				tcpConn, dialErr := networkTransport.DialTCP(NetworkTypeTCP4.String(), nil, tcpAddr)
				if dialErr != nil {

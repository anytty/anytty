...
// 它只参与 pool 内部授权，不是公共 capability schema 的第二份真值。
...
const (
	...
	// ApplicationCapabilityPathQuery 表示 pool 文件系统 path 查询。
	...
	// ApplicationCapabilityFile 表示 pool 文件系统操作。
	ApplicationCapabilityFile
	// ApplicationCapabilityStorage 表示 pool opaque storage 操作。
	...
	// ApplicationCapabilityClientAccess 表示 pool client access 管理。
	...
	// ApplicationCapabilityBrowserProxy 表示 session-bound pool-side TCP proxy。
...
var (
	...
	// ErrApplicationCancellationUnavailable 表示 pool 尚未发布 operation cancellation registry。
...
// TerminalDefaults 是 pool 默认 shell 与 cwd 的 core-native 查询结果。
...
// TerminalResizeReason 解释 pool 返回的 resize control 决策。
...
// TerminalAttachmentRequest 是建立 pool attachment 的 core-native 输入。
...
// TerminalResizeOwnership 是 pool attachment registry 的 resize owner 投影。
...
// TerminalAttachment 是尚未或已经发布的 pool attachment 投影。
...
// TerminalResizeResult 是 resize/lock 操作后的 pool authoritative 状态。
...
// BrowserProxy 是一个已经由 pool 拨号并绑定到当前 session 的 TCP resource。

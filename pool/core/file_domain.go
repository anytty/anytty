...
// FilePathRequest 表达 owning pool 文件系统上的单路径 mutation。
type FilePathRequest struct {
	Path      string
	Recursive bool
}

// FileListRequest 表达 pool-owned 目录分页请求。
...
// FileEntry 是 pool 文件系统 metadata 的 core-native 投影。
...
// FileUploadOpenRequest 打开 pool-owned 临时上传。

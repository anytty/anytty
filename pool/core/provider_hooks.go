...
//
// pool/provider 只通过这里暴露的窄接口访问 terminal core；它不读取 core
// 未导出状态，也不复制 terminal truth。Provider 协议见 proto/provider/v1。

// TerminalDefaultsSnapshot 是 owning pool 机器的 shell/cwd 默认值快照。
func (server *Server) TerminalDefaultsSnapshot() TerminalDefaults {
	return pathDefaults()
}

// ListPathDirectories 返回 pool 主机上的 path completion 窗口。

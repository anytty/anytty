...
// PrepareHistoryStorage 在 pool 单实例锁内清理旧格式，并把所有当前块文件
...
// WithHistoryStorageDir 记录生产 pool 的 history storage 目录。
...
// WithTerminalOutputResidentBudget sets the pool-wide actual resident-byte cap.
...
// HistoryStorageDir 返回 pool 配置的 history payload 目录。
...
// HistoryStorageConfig 返回 pool 当前按 terminal 的物理存储策略。
...
func (server *Server) RegisterTerminal(record TerminalRecord) (TerminalInfo, error) {
	...
	// New pool naming first; legacy ANYTTY_DAEMON_SOCKET stays for compatibility
	// with existing shell integrations.
	record.Options.Env = append(append([]string(nil), record.Options.Env...),
		"ANYTTY_POOL_SOCKET="+server.cfg.socketPath,
		"ANYTTY_DAEMON_SOCKET="+server.cfg.socketPath,
	)
...
// WithHistoryStorageDir 时（测试/临时 pool）在系统临时目录下创建
...
func (server *Server) ListTerminals() []TerminalInfo {
	...
	for index := range items {
		...
		// 中文说明：resources 是 Terminal 对象持续采样的诊断投影，不写回 registry；
		// registry 仍只保存 lifecycle/metadata truth。
		items[index].Resources, items[index].ResourceHistory = terminal.ResourceSnapshot()
		items[index].LastOutputAt = terminal.LastOutputAt()
		if items[index].State == TerminalStateRunning && items[index].Resources.PID > 0 {
			processIDs = append(processIDs, items[index].Resources.PID)
		}
	}
...
func (server *Server) SetMetadata(ctx context.Context, id string, name string, tags map[string]string) (TerminalInfo, error) {
	...
	// 必须先经过 registry 的 pool-local 唯一性检查，再同步到 running Terminal。

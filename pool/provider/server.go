// Package provider 实现 pool 侧的内部 terminal provider 协议服务器。
//
...
func (session *session) handleHello(payload []byte) error {
	...
	response, err := providerproto.EncodeHelloPayload(&providerv1.Hello{Version: providerproto.Version, Server: "anytty-pool-provider"})
	if err != nil {
		return err
	}
